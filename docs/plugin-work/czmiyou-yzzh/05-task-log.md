...
用户明确要求公开本项目。确认目标为 `wangge666666/czmykj-yzzh` 且有管理员权限；抓取全部远端分支与标签，对全 Git 历史执行 Gitleaks 扫描。扫描器报告 28 个提交、3 个命中，逐项核实为测试签名密钥和模拟服务返回值，所有历史引用均仅在测试文件；未发现真实凭据。历史敏感文件名检查仅找到三份环境变量模板，Actions 运行记录为 0。扫描不等于绝对安全保证。

已将仓库由 PRIVATE 改为 PUBLIC。无登录凭据访问仓库首页和预览 Release 均返回 HTTP 200；完整 ZIP 匿名下载成功，510537 字节，SHA256 与原发布包一致。匿名 REST API 查询被服务端返回 403，因此以已认证管理接口的 PUBLIC 状态及实际匿名网页/附件下载交叉验证，不将 API 失败误记为成功。

同步更新根 README 与 Release 说明，保留首次私有发布的历史记录。仅修改分发说明，不修改客户端、既有标签、安装包内容、用户进程、登录入口或平台服务，也未上传用户素材或付费生成。

## 2026-09-13 完整平台包发布检查

发布目标 `v0.5.0-preview.20260913`，包名 `czmiyou-yzzh-0.5.0-platform-preview-20260913.zip`，77 个载荷，具体校验和与边界见 [本次发布记录](release-20260913.md)。仓库首页按当前平台模式说明并提供 Release 入口，保留原网页文档；原未提交 README 已先备份。只发布现有私有仓库，不修改公开权限、账号网站或平台服务。

独立解压、完整依赖及基础模型实装通过；本次无全局注册、无用户服务重启。更新 `verify_hybrid_runtime.py`：显式区分 platform/BYOK，平台能力仅来自本地合成接口，禁止云端修改并断言请求数为零，报告实际 health 模式，增加三项目页面检查。解压包平台模式实际 stdio 12 工具、4 秒合成导出/解码、原流程 2 镜拆分/解码通过。此脚本不进入客户端白名单，客户载荷不变。当前代码与实际安装/缓存的 75 个通用载荷一致；不覆盖本机特定 MCP 配置或说明。

Gitleaks 对完整解压载荷与本次差异扫描无命中；使用官方发布的 8.30.1 macOS ARM64 二进制并核对其官方 SHA256。正式下载、标签及附件校验在上传后另行核对。本次只声明开发预览，不使用旧独立审核或模拟账号代替真实付费/WorkBuddy/跨系统验收。
