...
class WebAppTests(unittest.TestCase):
    ...
    def test_home_serves_project_center(self) -> None:
        ...
        self.assertIn("项目中心 · 米哟无限复刻", html)
    ...
    def test_partial_replacement_pages_share_ark_character_library(self) -> None:
        for route in ("/projects/person", "/projects/scene", "/projects/clothing"):
            with self.subTest(route=route):
                ...
                self.assertEqual(response.status_code, 200)
                self.assertIn('/static/character_library.js?v=20260915-sync1', response.get_data(as_text=True))
                response.close()
    ...
    def test_long_video_route_has_analysis_shots_and_final_preview(self) -> None:
        ...
        self.assertIn("非写实动画人偶 · 无身份特征", html)
        self.assertIn("宽松不透明白色长袖长裤工作服", html)
        self.assertIn("白色手套与平底鞋 · 全程完整穿着", html)
        ...
        self.assertIn('long_video.js?v=20260915-sync1', html)
    ...
    def test_web_generation_marks_success_after_automatic_download(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(job.output_path.resolve(), output.resolve())
