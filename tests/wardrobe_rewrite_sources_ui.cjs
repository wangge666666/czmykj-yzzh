...
async function run(){
 ...
 assert.match(r.$('generateReason').textContent,/人物/);change(r,'personAsset','asset://asset-person123456');assert.equal(r.$('generateBtn').disabled,false,'Clothing is optional when a character is selected');
 file(r,'clothingFile','衣服.png','image/png');assert.equal(r.$('generateBtn').disabled,false);assert.match(r.$('clothingPreview').src,/blob:/);
 assert.match(r.rows()[0].querySelector('.references').textContent,/@图片1 · 人物参考/);assert.match(r.rows()[0].querySelector('.references').textContent,/@图片2 · 人物1服装/);
