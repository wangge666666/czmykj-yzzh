const state = {
  config: null,
  jobId: null,
  activeJobId: null,
  shotCount: 0,
  actorCount: 1,
  pollTimer: null,
  lastJob: null,
  shotCasts: new Map(),
  manualCasts: new Set(),
  sceneGroups: [],
  shotScenes: new Map(),
  manualSceneShots: new Set(),
  nextSceneGroupId: 1,
  preserveSceneLibraryOnNextJob: false,
  castConfirmed: false,
  performanceDrafts: new Map(),
  positionDrafts: new Map(),
  dirtyActorReferences: new Set(),
  characterLibrary: { configured: false, upload_ready: false, groups: [], assets: [], message: "" },
  pendingActorUploadIndex: null,
  archives: [],
  activeWorkflowStep: "source",
  draftTimer: null,
  draftReady: false,
  busy: false,
  confirmResolver: null,
};

const $ = (selector) => document.querySelector(selector);
const LONG_API = "/api/real-long-video";
const REAL_DEFAULT_FINAL_STYLE = "ultra_realistic";
const REAL_DRAFT_CACHE_KEY = "depthflow.realLongVideo.draft.v2";
const WHOLE_GENERATION_STRATEGY = "whole_white_model";
const PER_SHOT_GENERATION_STRATEGY = "per_shot";
const defaultRoles = ["画面主要人物", "画面第二人物", "画面第三人物", "画面第四人物"];
let shotPreviewLoadToken = 0;
let quickCharacterGroupTarget = "#arkCharacterGroup";

function escapeHtml(value) {
  return String(value ?? "").replace(/[&<>'"]/g, (character) => ({
    "&": "&amp;", "<": "&lt;", ">": "&gt;", "'": "&#39;", '"': "&quot;",
  })[character]);
}

function toast(message, isError = false) {
  const node = $("#toast");
  node.textContent = message;
  node.classList.toggle("error", isError);
  node.classList.add("show");
  clearTimeout(node._timer);
  node._timer = setTimeout(() => node.classList.remove("show"), 3800);
}

function confirmAction(message, options = {}) {
  if (state.confirmResolver) state.confirmResolver(false);
  return new Promise((resolve) => {
    const overlay = document.createElement("div");
    overlay.className = "app-confirm-overlay";
    const dialog = document.createElement("section");
    dialog.className = `app-confirm-dialog${options.danger ? " danger" : ""}`;
    dialog.setAttribute("role", "dialog");
    dialog.setAttribute("aria-modal", "true");
    dialog.setAttribute("aria-labelledby", "appConfirmTitle");
    const title = document.createElement("h3");
    title.id = "appConfirmTitle";
    title.textContent = options.title || "请确认本次操作";
    const content = document.createElement("div");
    content.className = "app-confirm-message";
    content.textContent = String(message || "");
    const actions = document.createElement("div");
    actions.className = "app-confirm-actions";
    const cancelButton = document.createElement("button");
    cancelButton.type = "button";
    cancelButton.className = "app-confirm-cancel";
    cancelButton.textContent = options.cancelLabel || "取消";
    const confirmButton = document.createElement("button");
    confirmButton.type = "button";
    confirmButton.className = "app-confirm-accept";
    confirmButton.textContent = options.confirmLabel || "确认继续";
    actions.append(cancelButton, confirmButton);
    dialog.append(title, content, actions);
    overlay.append(dialog);

    const finish = (confirmed) => {
      if (!overlay.isConnected) return;
      document.removeEventListener("keydown", onKeyDown);
      overlay.remove();
      document.body.classList.remove("app-confirm-open");
      if (state.confirmResolver === finish) state.confirmResolver = null;
      resolve(Boolean(confirmed));
    };
    const onKeyDown = (event) => {
      if (event.key === "Escape") finish(false);
    };
    state.confirmResolver = finish;
    cancelButton.addEventListener("click", () => finish(false));
    confirmButton.addEventListener("click", () => finish(true));
    overlay.addEventListener("click", (event) => {
      if (event.target === overlay) finish(false);
    });
    document.addEventListener("keydown", onKeyDown);
    document.body.append(overlay);
    document.body.classList.add("app-confirm-open");
    window.requestAnimationFrame(() => confirmButton.focus());
  });
}

async function api(url, options = {}) {
  const response = await fetch(url, options);
  let payload;
  try { payload = await response.json(); } catch (_error) { payload = {}; }
  if (!response.ok) throw new Error(window.DepthFlowUI?.formatApiError?.(payload, response) || payload.error || payload.message || `请求失败（HTTP ${response.status}）`);
  return payload;
}

function finalModelSpec(model = $("#model")?.value) {
  return (state.config?.real_final_video_models || []).find((item) => item.model === model) || null;
}

function syncFinalModelOptions(preferredResolution = "") {
  const select = $("#model");
  const resolution = $("#resolution");
  const ratio = $("#ratio");
  if (!select || !resolution || !ratio) return;
  const spec = finalModelSpec(select.value) || {
    label: select.value.includes("2-5") ? "Seedance 2.5" : "Seedance 2.0",
    resolutions: select.value.includes("2-5") ? ["480p", "720p"] : ["480p", "720p", "1080p"],
  };
  const previous = preferredResolution || resolution.value || "720p";
  resolution.innerHTML = spec.resolutions.map((value) => `<option value="${value}">${value}</option>`).join("");
  resolution.value = spec.resolutions.includes(previous) ? previous : (spec.resolutions.includes("720p") ? "720p" : spec.resolutions[0]);
  $("#selectedSeedanceLabel").textContent = spec.label;
  $("#resolutionHint").textContent = `${spec.label} 支持${spec.resolutions.join("、")}`;
  const videoEditing25 = select.value.includes("seedance-2-5");
  if (videoEditing25) {
    if (ratio.value !== "adaptive") ratio.dataset.previousManualRatio = ratio.value;
    ratio.value = "adaptive";
    ratio.disabled = true;
    $("#ratioHint").textContent = "Seedance 2.5 带白模时固定 adaptive，输出比例跟随白模视频";
  } else {
    ratio.disabled = false;
    if (ratio.dataset.previousManualRatio) ratio.value = ratio.dataset.previousManualRatio;
    $("#ratioHint").textContent = "Seedance 2.0 可人工选择；adaptive 会跟随白模视频";
  }
}

function generationStrategy() {
  return document.querySelector('input[name="generationStrategy"]:checked')?.value || PER_SHOT_GENERATION_STRATEGY;
}

function renderWholeSceneOptions() {
  const select = $("#wholeSceneReference");
  if (!select) return;
  const previous = select.value;
  const options = [];
  state.sceneGroups.forEach((group) => {
    group.images.forEach((image) => {
      options.push({
        value: `${group.id}:${Number(image.index)}`,
        label: `${group.name || "未命名场景"} · ${image.name || `角度 ${image.index}`}`,
      });
    });
  });
  select.innerHTML = options.length
    ? options.map((item) => `<option value="${escapeHtml(item.value)}">${escapeHtml(item.label)}</option>`).join("")
    : '<option value="">请先在步骤04上传场景图片</option>';
  if (options.some((item) => item.value === previous)) select.value = previous;
  else if (options.length) select.value = options[0].value;
}

function syncGenerationStrategy({ keepResolution = true } = {}) {
  const whole = generationStrategy() === WHOLE_GENERATION_STRATEGY;
  document.querySelectorAll(".generation-strategy-card").forEach((card) => {
    card.classList.toggle("active", card.querySelector("input")?.checked === true);
  });
  $("#wholeGenerationControls")?.classList.toggle("hidden", !whole);
  document.querySelector(".seedream-section")?.classList.toggle("strategy-disabled", whole);
  const seedanceModeTitle = document.querySelector(".seedance-section .model-section-title b");
  if (seedanceModeTitle) seedanceModeTitle.textContent = whole ? "整段角色与场景连续重绘" : "按分镜人数动态复刻";
  const styleScope = document.querySelector(".final-style-heading>span");
  if (styleScope) styleScope.textContent = whole ? "全片一致" : "逐镜一致";
  const commonPromptLabel = document.querySelector('label[for="prompt"]');
  if (commonPromptLabel) commonPromptLabel.textContent = whole ? "整段成片共用的动作与连续性约束" : "所有分镜共用的动作与构图约束";
  $("#regenerateAllBtn")?.classList.toggle("hidden", whole);
  if (whole) {
    const seedance25 = (state.config?.real_final_video_models || []).find((item) => item.id === "seedance_2_5");
    if (seedance25 && $("#model").value !== seedance25.model) $("#model").value = seedance25.model;
    syncFinalModelOptions(keepResolution ? $("#resolution").value : "720p");
    $("#model").disabled = true;
    $("#costNote").textContent = "整段模式只提交 1 次 Seedance 2.5；不调用 Seedream 场景板";
  } else {
    $("#model").disabled = false;
    syncFinalModelOptions(keepResolution ? $("#resolution").value : "720p");
  }
  renderWholeSceneOptions();
  updateCostAndButton();
  scheduleDraftCache();
}

function realCharacterGroups() {
  return (state.characterLibrary?.groups || []).filter((group) => group.group_type === "AIGC");
}

function syncCharacterGroupActions() {
  const selected = $("#arkCharacterGroup")?.value || "";
  const group = realCharacterGroups().find((item) => item.id === selected);
  $("#renameCharacterGroupBtn").disabled = !group || group.can_delete === false;
  $("#deleteCharacterGroupBtn").disabled = !group || group.can_delete === false;
  $("#uploadCharacterAssetBtn").disabled = !group || group.can_upload === false;
}

function renderCharacterLibrary() {
  const library = state.characterLibrary || { groups: [], assets: [] };
  const groups = realCharacterGroups();
  const previousGroup = $("#arkCharacterGroup")?.value || "";
  const groupById = new Map(groups.map((group) => [group.id, group]));
  const assets = (library.assets || []).filter((asset) => groupById.has(asset.group_id));
  $("#arkCharacterLibraryState").textContent = library.configured
    ? `${groups.length} 个 AIGC 虚拟人像组 · ${assets.length} 个素材 · ${library.project_name || "default"} 项目`
    : "素材库未配置";
  $("#arkCharacterGroup").innerHTML = groups.length
    ? `<option value="">选择 AIGC 虚拟人像组</option>${groups.map((group) => `<option value="${escapeHtml(group.id)}">${escapeHtml(group.name)}</option>`).join("")}`
    : `<option value="">暂无 AIGC 虚拟人像组</option>`;
  if (previousGroup && groups.some((group) => group.id === previousGroup)) $("#arkCharacterGroup").value = previousGroup;
  // Keep the action clickable so missing entitlement or AK/SK produces a precise message.
  $("#uploadCharacterAssetBtn").disabled = false;
  $("#arkCharacterLibraryHint").textContent = library.message || (library.upload_ready
    ? (library.storage_mode === "tos"
      ? "使用私有 TOS 上传人物图片；审核完成后自动清理临时对象。"
      : "TOS 尚未开通，已自动使用项目自身的一次性加密图片通道。")
    : "需先开通高级创作入门版或更高权益并配置素材库 AK/SK。");
  const items = assets.map((asset) => {
    const group = groupById.get(asset.group_id);
    const preview = asset.url
      ? `<button type="button" class="ark-character-asset-preview" data-real-reference-preview-url="${escapeHtml(asset.url)}" data-real-reference-preview-caption="${escapeHtml(asset.name)}"><img src="${escapeHtml(asset.url)}" alt="${escapeHtml(asset.name)}" loading="lazy"><span>预览</span></button>`
      : `<div class="asset-placeholder">ARK</div>`;
    return `<article class="ark-character-asset">${preview}<div class="ark-character-asset-copy"><b title="${escapeHtml(asset.name)}">${escapeHtml(asset.name)}</b><small>${escapeHtml(group?.name || asset.group_id || "未分组")} · ${escapeHtml(asset.status || "未知")}</small><small class="ark-character-asset-uri" title="${escapeHtml(asset.uri)}">${escapeHtml(asset.uri)}</small></div>${asset.can_delete === false ? `<small>公司共享角色</small>` : `<button type="button" data-delete-character-asset="${escapeHtml(asset.id)}">删除</button>`}</article>`;
  }).join("");
  $("#arkCharacterAssetList").innerHTML = items || `<div class="scene-library-empty">暂无 AIGC 虚拟人像素材；可先在上方创建虚拟人像组。</div>`;
  bindActorReferencePreviews($("#arkCharacterAssetList"));
  syncCharacterGroupActions();
}

function characterGroupOptions(selected = "") {
  const groups = realCharacterGroups().filter((group) => group.can_upload !== false);
  const rows = [`<option value="">选择 AIGC 虚拟人像组</option>`];
  for (const group of groups) {
    rows.push(`<option value="${escapeHtml(group.id)}" ${group.id === selected ? "selected" : ""}>${escapeHtml(group.name)}</option>`);
  }
  return rows.join("");
}

function canManageCharacterAsset(uri) {
  const asset = (state.characterLibrary?.assets || []).find((item) => item.uri === uri);
  return asset ? asset.can_delete !== false : state.characterLibrary?.storage_mode !== "platform";
}

function syncActorLibraryUploadControl(index) {
  const card = document.querySelector(`[data-actor-index="${index}"]`);
  const fields = card?.querySelector(".actor-library-upload-fields");
  const group = document.querySelector(`#actorArkGroup${index}`);
  const hint = document.querySelector(`#actorArkUploadHint${index}`);
  if (!card || !fields || !hint) return;
  const usesAsset = document.querySelector(`#actorSource${index}`)?.value === "ark_asset";
  const libraryReady = Boolean(state.characterLibrary?.upload_ready && realCharacterGroups().some((group) => group.can_upload !== false));
  const running = state.busy || ["queued", "running"].includes(state.lastJob?.status);
  const uploadButton = card.querySelector("[data-upload-actor-character]");
  const deleteButton = card.querySelector("[data-delete-actor-character]");
  const boundAssetUri = existingActor(index)?.asset_uri || existingActor(index)?.ark_library_asset_uri || "";
  fields.classList.toggle("hidden", usesAsset);
  if (group) group.disabled = !libraryReady;
  if (uploadButton) {
    uploadButton.disabled = usesAsset || running;
    uploadButton.title = usesAsset
      ? "当前人物使用角色库已有角色；如需上传新人物，请先点击“上传新人物”。"
      : running
        ? "当前任务运行中，请完成或暂停后再上传。"
        : libraryReady
          ? "将本人物图片单独上传到火山角色库并等待变为 Active。"
          : "当前人物库尚未就绪；请查看顶部服务状态。";
  }
  if (deleteButton) {
    const hasBoundAsset = /^asset:\/\/asset-[A-Za-z0-9_-]{6,120}$/.test(boundAssetUri) && canManageCharacterAsset(boundAssetUri);
    deleteButton.disabled = running || !hasBoundAsset;
    deleteButton.dataset.assetUri = hasBoundAsset ? boundAssetUri : "";
    deleteButton.title = hasBoundAsset
      ? "永久删除该人物当前绑定的火山角色素材。"
      : "该人物尚未绑定可删除的火山角色素材。";
  }
  if (usesAsset) {
    hint.textContent = "最终成片将直接引用所选的 Active 火山角色 Asset。";
  } else if (!libraryReady) {
    hint.textContent = state.characterLibrary?.message || "需先配置火山素材库 AK/SK，才能上传人物。";
  } else {
    hint.textContent = "点击下方“上传并绑定火山角色库”后，系统会等待素材变为 Active；不会生成遮眼图或素描图。";
  }
}

function refreshActorLibraryUploadOptions() {
  for (let index = 1; index <= state.actorCount; index += 1) {
    const select = document.querySelector(`#actorArkGroup${index}`);
    if (select) {
      const current = select.value;
      select.innerHTML = characterGroupOptions(current);
      if (current && [...select.options].some((option) => option.value === current)) select.value = current;
    }
    syncActorLibraryUploadControl(index);
  }
}

async function loadCharacterLibrary({ rerenderActors = false } = {}) {
  try {
    $("#refreshCharacterLibraryBtn").disabled = true;
    state.characterLibrary = await api(`${LONG_API}/character-library`);
    renderCharacterLibrary();
    if (rerenderActors) {
      refreshActorAssetOptions();
      refreshActorLibraryUploadOptions();
    }
  } catch (error) {
    state.characterLibrary = { configured: false, upload_ready: false, groups: [], assets: [], message: error.message };
    renderCharacterLibrary();
    refreshActorLibraryUploadOptions();
    toast(error.message, true);
  } finally {
    $("#refreshCharacterLibraryBtn").disabled = false;
  }
}

async function createCharacterGroup(name, description, targetSelector = "#arkCharacterGroup") {
  if (!state.characterLibrary?.configured) return toast(state.characterLibrary?.message || "尚未配置火山素材库 AK/SK。", true);
  if (!name) return toast("请填写新 AIGC 虚拟人像组名称。", true);
  try {
    document.querySelectorAll("#createRealCharacterGroupBtn, #confirmCreateCharacterGroupBtn, [data-create-character-group]").forEach((button) => { button.disabled = true; });
    const result = await api(`${LONG_API}/character-library/groups`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name, description }),
    });
    await loadCharacterLibrary({ rerenderActors: true });
    if ([...$("#arkCharacterGroup").options].some((option) => option.value === result.group_id)) $("#arkCharacterGroup").value = result.group_id;
    const target = document.querySelector(targetSelector);
    if (target && [...target.options].some((option) => option.value === result.group_id)) {
      target.value = result.group_id;
      target.dispatchEvent(new Event("change", { bubbles: true }));
    }
    syncCharacterGroupActions();
    toast(result.message || "AIGC 虚拟人像组已创建。", false);
    return true;
  } catch (error) {
    toast(error.message, true);
    return false;
  } finally {
    document.querySelectorAll("#createRealCharacterGroupBtn, #confirmCreateCharacterGroupBtn, [data-create-character-group]").forEach((button) => { button.disabled = false; });
  }
}

async function createRealCharacterGroup() {
  const name = $("#arkNewGroupName").value.trim();
  const description = $("#arkNewGroupDescription").value.trim();
  const created = await createCharacterGroup(name, description);
  if (!created) return;
  $("#arkNewGroupName").value = "";
  $("#arkNewGroupDescription").value = "";
}

function openQuickCharacterGroupDialog(targetSelector = "#arkCharacterGroup") {
  if (!state.characterLibrary?.configured) return toast(state.characterLibrary?.message || "尚未配置火山素材库 AK/SK。", true);
  quickCharacterGroupTarget = targetSelector;
  $("#quickCharacterGroupName").value = "";
  $("#quickCharacterGroupDescription").value = "";
  $("#quickCharacterGroupDialog").showModal();
  window.setTimeout(() => $("#quickCharacterGroupName").focus(), 0);
}

function closeQuickCharacterGroupDialog() {
  if ($("#quickCharacterGroupDialog").open) $("#quickCharacterGroupDialog").close();
}

async function confirmQuickCharacterGroup() {
  const name = $("#quickCharacterGroupName").value.trim();
  const description = $("#quickCharacterGroupDescription").value.trim();
  const created = await createCharacterGroup(name, description, quickCharacterGroupTarget);
  if (created) closeQuickCharacterGroupDialog();
}

async function renameSelectedCharacterGroup() {
  if (realCharacterGroups().find((item) => item.id === $("#arkCharacterGroup").value)?.can_delete === false) return toast("公司共享角色请在画布或创作中心管理。", true);
  const groupId = $("#arkCharacterGroup").value;
  const group = realCharacterGroups().find((item) => item.id === groupId);
  if (!group) return toast("请先选择要重命名的 AIGC 虚拟人像组。", true);
  const name = window.prompt("新的角色组名称：", group.name || "");
  if (name === null) return;
  const cleanName = name.trim();
  if (!cleanName) return toast("角色组名称不能为空。", true);
  const description = window.prompt("角色组说明（可留空）：", group.description || "");
  if (description === null) return;
  try {
    await api(`${LONG_API}/character-library/groups/${encodeURIComponent(groupId)}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: cleanName, description: description.trim() }),
    });
    await loadCharacterLibrary({ rerenderActors: true });
    $("#arkCharacterGroup").value = groupId;
    syncCharacterGroupActions();
    toast("AIGC 虚拟人像组已重命名。", false);
  } catch (error) { toast(error.message, true); }
}

async function deleteSelectedCharacterGroup() {
  const groupId = $("#arkCharacterGroup").value;
  const group = realCharacterGroups().find((item) => item.id === groupId);
  if (!group) return toast("请先选择要删除的 AIGC 虚拟人像组。", true);
  if (group.can_delete === false) return toast("公司共享角色请在画布或创作中心管理。", true);
  const assetCount = (state.characterLibrary.assets || []).filter((asset) => asset.group_id === groupId).length;
  if (assetCount) return toast(`该角色组还有 ${assetCount} 个素材，请先逐个删除素材，再删除角色组。`, true);
  if (!(await confirmAction(`确定永久删除 AIGC 虚拟人像组“${group.name}”吗？\n${groupId}\n\n该操作不可撤销。`, { title: "删除虚拟人像组", confirmLabel: "永久删除", danger: true }))) return;
  try {
    await api(`${LONG_API}/character-library/groups/${encodeURIComponent(groupId)}`, { method: "DELETE" });
    await loadCharacterLibrary({ rerenderActors: true });
    toast(`已删除 AIGC 虚拟人像组：${group.name}`, false);
  } catch (error) { toast(error.message, true); }
}

async function uploadCharacterAsset() {
  const groupId = $("#arkCharacterGroup").value;
  const name = $("#arkCharacterName").value.trim();
  const file = $("#arkCharacterFile").files[0];
  if (!state.characterLibrary?.configured) return toast(state.characterLibrary?.message || "尚未配置火山素材库 AK/SK。", true);
  if (!groupId) return toast("请选择要上传到的火山人物角色组。", true);
  if (!state.characterLibrary?.upload_ready) return toast("当前还缺少火山素材库 AK/SK，无法提交人物图片。", true);
  if (!name) return toast("请填写人物素材名称。", true);
  if (!file) return toast("请选择要上传的人物图片。", true);
  try {
    $("#uploadCharacterAssetBtn").disabled = true;
    const form = new FormData();
    form.append("group_id", groupId); form.append("name", name); form.append("asset_file", file);
    const result = await api(`${LONG_API}/character-library/assets`, { method: "POST", body: form });
    toast(result.message || "人物素材已提交火山入库。");
    $("#arkCharacterName").value = ""; $("#arkCharacterFile").value = ""; $("#arkCharacterFileName").textContent = "JPG / PNG / WEBP，最大30MB";
    await loadCharacterLibrary({ rerenderActors: true });
  } catch (error) { toast(error.message, true); }
  finally { $("#uploadCharacterAssetBtn").disabled = false; }
}

async function deleteCharacterAsset(assetId) {
  if ((state.characterLibrary.assets || []).find((item) => item.id === assetId)?.can_delete === false) return toast("公司共享角色请在画布或创作中心管理。", true);
  const asset = (state.characterLibrary.assets || []).find((item) => item.id === assetId);
  if (!asset) return;
  if (!(await confirmAction(`确定从火山人物库永久删除“${asset.name}”吗？\n${asset.uri}\n\n该操作不可撤销，已引用此素材的旧任务可能无法再次生成。`, { title: "删除火山人物素材", confirmLabel: "永久删除", danger: true }))) return;
  try {
    await api(`${LONG_API}/character-library/assets/${encodeURIComponent(assetId)}`, { method: "DELETE" });
    toast(`已从火山人物库删除：${asset.name}`);
    await loadCharacterLibrary({ rerenderActors: true });
  } catch (error) { toast(error.message, true); }
}

function badge(selector, ready, yes, no) {
  const node = $(selector);
  node.className = `badge ${ready ? "ready" : "missing"}`;
  node.innerHTML = `<i></i>${ready ? yes : no}`;
}

function bind(inputSelector, labelSelector) {
  $(inputSelector).addEventListener("change", () => {
    const file = $(inputSelector).files[0];
    if (file) $(labelSelector).textContent = file.name;
  });
}

function existingActor(index) {
  return (state.lastJob?.actors || []).find((actor) => Number(actor.id) === index) || null;
}

function whiteModelQualityReady(shot) {
  if (!shot?.has_white_model) return false;
  // Legacy projects are allowed to reach the submit preflight, where the backend
  // performs the new local audit before any paid final task can be created.
  if (!shot?.white_model_qa) return true;
  return Boolean(shot.white_model_qa.passed === true || shot.white_model_manually_approved === true);
}

function realReferencePreviewMarkup(index, existing = null) {
  if (actorUsesAsset(index, existing)) {
    const uri = actorAssetUri(index, existing);
    const asset = characterAssetByUri(uri);
    return `<div class="real-reference-title"><div><b>最终人物身份来源</b><small>仅使用火山角色库 Asset，不生成遮眼图和素描图</small></div><span>火山人物库</span></div>
      <div class="real-reference-asset-summary"><div><b>${escapeHtml(asset?.name || `人物 ${index} 当前绑定角色`)}</b><small title="${escapeHtml(uri)}">${escapeHtml(uri || "请选择 Active 人物素材")}</small></div><span>${asset?.status === "Active" ? "Active · 可用于成片" : "当前已绑定"}</span></div>`;
  }
  const status = existing?.ark_library_upload_status || (state.dirtyActorReferences.has(index) ? "等待上传" : "未入库");
  const uri = existing?.ark_library_asset_uri || "";
  return `<div class="real-reference-title"><div><b>火山角色库入库状态</b><small>人物原图只用于创建角色库素材，不会直接提交最终成片</small></div><span>${escapeHtml(status)}</span></div>
    <div class="real-reference-placeholder"><i>ARK</i><b>${escapeHtml(uri || "请上传人物图并绑定角色库")}</b><small>${escapeHtml(existing?.ark_library_upload_error || "审核通过后会自动绑定 Asset；无需遮眼图和素描图。")}</small></div>`;
}

function actorUsesAsset(index, existing = null) {
  const source = document.querySelector(`#actorSource${index}`)?.value;
  if (source) return source === "ark_asset";
  return Boolean(existing?.uses_authorized_asset || existing?.asset_uri);
}

function actorAssetUri(index, existing = null) {
  const selected = document.querySelector(`#actorAsset${index}`)?.value?.trim();
  const manual = document.querySelector(`#actorAssetManual${index}`)?.value?.trim();
  return selected || manual || existing?.asset_uri || "";
}

function characterAssetByUri(uri) {
  return (state.characterLibrary.assets || []).find((asset) => asset.uri === uri) || null;
}

function actorAssetPreviewMarkup(index, uri = "") {
  const asset = characterAssetByUri(uri);
  if (!uri) {
    return `<div class="actor-selected-preview empty"><i>ARK</i><span>选择角色后<br>在这里预览</span></div>`;
  }
  const group = realCharacterGroups().find((item) => item.id === asset?.group_id);
  const name = asset?.name || `人物 ${index} 当前绑定角色`;
  const localPreviewUrl = existingActor(index)?.person_preview_url || "";
  const previewUrl = asset?.url || localPreviewUrl;
  const media = previewUrl
    ? `<button type="button" class="actor-selected-preview-image" data-real-reference-preview-url="${escapeHtml(previewUrl)}" data-real-reference-preview-caption="${escapeHtml(name)}"><img src="${escapeHtml(previewUrl)}" alt="${escapeHtml(name)}" loading="lazy"><span>${asset?.url ? "点击查看大图" : "本地人物预览"}</span></button>`
    : `<div class="actor-selected-preview-fallback"><i>ARK</i><span>暂无在线缩略图</span></div>`;
  const sourceNote = localPreviewUrl && !asset?.url
    ? `${group?.name || "火山 AIGC 角色库"} · 本地原图回退`
    : (group?.name || "火山 AIGC 角色库");
  return `<div class="actor-selected-preview-card">${media}<div><b title="${escapeHtml(name)}">${escapeHtml(name)}</b><small>${escapeHtml(sourceNote)}</small><small title="${escapeHtml(uri)}">${escapeHtml(uri)}</small></div></div>`;
}

function savedActorReferencePreviewMarkup(index, kind, existing = null) {
  const url = kind === "clothing" ? existing?.clothing_preview_url : existing?.person_preview_url;
  if (!url) return "";
  const caption = kind === "clothing" ? `人物 ${index} 已保存服装` : `人物 ${index} 已保存人物图`;
  return `<button type="button" class="actor-saved-reference-preview" data-real-reference-preview-url="${escapeHtml(url)}" data-real-reference-preview-caption="${escapeHtml(caption)}"><img src="${escapeHtml(url)}" alt="${escapeHtml(caption)}" loading="lazy"><span>点击预览</span></button>`;
}

function renderActorAssetPreview(index) {
  const host = document.querySelector(`#actorAssetPreview${index}`);
  if (!host) return;
  host.innerHTML = actorAssetPreviewMarkup(index, actorAssetUri(index, existingActor(index)));
  bindActorReferencePreviews(host);
}

function characterAssetOptions(existing = null) {
  const active = (state.characterLibrary.assets || []).filter((asset) => asset.status === "Active" && (!asset.asset_type || asset.asset_type === "Image"));
  const existingUri = existing?.asset_uri || "";
  const rows = [`<option value="">选择已授权人物素材</option>`];
  for (const asset of active) {
    rows.push(`<option value="${escapeHtml(asset.uri)}" ${asset.uri === existingUri ? "selected" : ""}>${escapeHtml(asset.name)} · ${escapeHtml(asset.id)}</option>`);
  }
  if (existingUri && !active.some((asset) => asset.uri === existingUri)) {
    rows.push(`<option value="${escapeHtml(existingUri)}" selected>当前已绑定 · ${escapeHtml(existingUri)}</option>`);
  }
  return rows.join("");
}

function refreshActorAssetOptions() {
  for (let index = 1; index <= state.actorCount; index += 1) {
    const select = document.querySelector(`#actorAsset${index}`);
    if (!select) continue;
    const current = select.value || existingActor(index)?.asset_uri || "";
    select.innerHTML = characterAssetOptions({ asset_uri: current });
    select.value = current;
    renderActorAssetPreview(index);
  }
}

function bindActorReferencePreviews(root = document) {
  root.querySelectorAll?.("[data-real-reference-preview-url]").forEach((button) => {
    if (button.dataset.previewBound) return;
    button.dataset.previewBound = "1";
    button.addEventListener("click", () => {
      window.DepthFlowUploadPreview?.open(
        `${button.dataset.realReferencePreviewUrl}?v=${Date.now()}`,
        "image",
        button.dataset.realReferencePreviewCaption || "火山人物角色预览",
      );
    });
  });
}

function renderActorReferencePreviews(existingActors = []) {
  for (let index = 1; index <= state.actorCount; index += 1) {
    const host = document.querySelector(`[data-actor-index="${index}"] .real-reference-section`);
    if (!host) continue;
    const existing = existingActors.find((actor) => Number(actor.id) === index) || null;
    host.innerHTML = realReferencePreviewMarkup(index, existing);
    bindActorReferencePreviews(host);
  }
}

function actorTemplate(index, existing = null) {
  const role = existing?.role || defaultRoles[index - 1];
  const personNote = existing?.has_person && !existing?.uses_authorized_asset ? "已保存人物原图，可上传角色库" : "选择拥有合法授权的人物图片";
  const clothingNote = existing?.has_clothing ? "已保存服装素材，可直接复用" : "建议白底服装参考";
  const usesAsset = Boolean(existing?.uses_authorized_asset || existing?.asset_uri);
  const libraryStatus = existing?.ark_library_upload_status || "";
  const libraryAssetName = existing?.ark_library_asset_name || `人物${index}·${role}`;
  const libraryStatusNote = libraryStatus
    ? `上次角色库提交：${libraryStatus}${existing?.ark_library_asset_uri ? ` · ${existing.ark_library_asset_uri}` : ""}`
    : "人物图必须先上传角色库并变为 Active，才能生成成片。";
  const boundAssetUri = existing?.asset_uri || existing?.ark_library_asset_uri || "";
  const canDeleteBoundAsset = /^asset:\/\/asset-[A-Za-z0-9_-]{6,120}$/.test(boundAssetUri) && canManageCharacterAsset(boundAssetUri);
  return `
    <article class="long-actor-card" data-actor-index="${index}">
      <div class="long-actor-head"><div><span class="long-actor-index">${String(index).padStart(2, "0")}</span><b>人物 ${index}</b><small>跨分镜固定身份</small></div><small>每个分镜按需选择</small></div>
      <div class="long-actor-body">
        <div class="field role-field"><label for="roleDescription${index}">原片角色定位</label><input id="roleDescription${index}" type="text" maxlength="200" value="${escapeHtml(role)}"><small>例如：画面左侧短发人物</small></div>
        <div class="long-actor-asset"><b>人物身份来源</b><input id="actorSource${index}" type="hidden" value="${usesAsset ? "ark_asset" : "local_library"}"><div class="actor-source-buttons" role="group" aria-label="人物${index}身份来源"><button type="button" data-actor-source-mode="local_library" class="${usesAsset ? "" : "active"}">上传新人物</button><button type="button" data-actor-source-mode="ark_asset" class="${usesAsset ? "active" : ""}">使用角色库已有角色</button></div><div class="field actor-asset-reference ${usesAsset ? "" : "hidden"}"><label for="actorAsset${index}">AIGC 虚拟角色</label><div class="actor-asset-choice-grid"><div class="actor-asset-select-controls"><select id="actorAsset${index}">${characterAssetOptions(existing)}</select><small>仅 Active 素材可直接用于成片</small><details class="actor-asset-manual"><summary>手动输入 Asset ID（高级）</summary><input id="actorAssetManual${index}" type="text" value="${escapeHtml(existing?.asset_uri || "")}" placeholder="asset://asset-..."></details></div><div id="actorAssetPreview${index}" class="actor-selected-preview-host">${actorAssetPreviewMarkup(index, existing?.asset_uri || "")}</div></div></div><div class="actor-local-reference ${usesAsset ? "hidden" : ""}"><label class="mini-drop" for="personImage${index}"><input id="personImage${index}" type="file" accept="image/jpeg,image/png,image/webp,.jpg,.jpeg,.png,.webp"><span>选择虚拟人物图片</span><small id="personFileName${index}" class="${existing?.has_person && !usesAsset ? "saved-reference" : ""}">${personNote}</small></label><div class="actor-library-optin"><div class="actor-library-upload-fields"><div class="field"><label for="actorArkGroup${index}">上传到 AIGC 人像组</label><div class="character-group-picker"><select id="actorArkGroup${index}">${characterGroupOptions(existing?.ark_library_group_id || "")}</select><button type="button" class="ghost-button" data-create-character-group="#actorArkGroup${index}">＋ 创建人像组</button></div></div><div class="field"><label for="actorArkName${index}">角色素材名称</label><input id="actorArkName${index}" type="text" maxlength="64" value="${escapeHtml(libraryAssetName)}" data-auto-name="${existing?.ark_library_asset_name ? "0" : "1"}"></div></div><small id="actorArkUploadHint${index}" class="actor-library-upload-hint">${escapeHtml(libraryStatusNote)}</small></div></div></div>
        <div class="long-actor-asset"><b>对应服装</b><div class="actor-upload-preview-grid ${existing?.clothing_preview_url ? "has-preview" : ""}"><label class="mini-drop" for="clothingImage${index}"><input id="clothingImage${index}" type="file" accept="image/*,.heic,.heif"><span>选择服装图片</span><small id="clothingFileName${index}" class="${existing?.has_clothing ? "saved-reference" : ""}">${clothingNote}</small></label>${savedActorReferencePreviewMarkup(index, "clothing", existing)}</div></div>
      </div>
      <div class="actor-character-action-row">
        <button type="button" class="secondary-button" data-upload-actor-character="${index}">上传此人物到角色库</button>
        <button type="button" class="ghost-button actor-character-delete" data-delete-actor-character="${index}" data-asset-uri="${escapeHtml(canDeleteBoundAsset ? boundAssetUri : "")}" ${canDeleteBoundAsset ? "" : "disabled"}>从角色库删除此人物</button>
        <small>上传和删除只作用于人物 ${index}；删除前会再次确认，且不会连带修改虚拟人物项目。</small>
      </div>
      <div class="real-reference-section">${realReferencePreviewMarkup(index, existing)}</div>
    </article>`;
}

function bindActor(index) {
  $(`#roleDescription${index}`).addEventListener("input", () => {
    document.querySelectorAll(`[data-person-mapping] option[value="${index}"]`).forEach((node) => {
      node.textContent = `人物${index} · ${$(`#roleDescription${index}`).value.trim() || `人物${index}`}`;
    });
    const arkName = $(`#actorArkName${index}`);
    if (arkName?.dataset.autoName === "1") arkName.value = `人物${index}·${$(`#roleDescription${index}`).value.trim() || `人物${index}`}`;
    invalidateCastConfirmation();
  });
  bind(`#personImage${index}`, `#personFileName${index}`);
  bind(`#clothingImage${index}`, `#clothingFileName${index}`);
  $(`#personImage${index}`).addEventListener("change", () => {
    if ($(`#personImage${index}`).files[0]) state.dirtyActorReferences.add(index);
    else state.dirtyActorReferences.delete(index);
    renderActorReferencePreviews(state.lastJob?.actors || []);
    syncActorLibraryUploadControl(index);
    updateCostAndButton();
  });
  bindActorReferencePreviews($(`[data-actor-index="${index}"]`));
  const sourceSelect = $(`#actorSource${index}`);
  const syncSource = () => {
    const card = $(`[data-actor-index="${index}"]`);
    const usesAsset = sourceSelect.value === "ark_asset";
    card.querySelectorAll("[data-actor-source-mode]").forEach((button) => {
      button.classList.toggle("active", button.dataset.actorSourceMode === sourceSelect.value);
    });
    card.querySelectorAll(".actor-local-reference").forEach((node) => node.classList.toggle("hidden", usesAsset));
    card.querySelector(".actor-asset-reference")?.classList.toggle("hidden", !usesAsset);
    if (usesAsset) state.dirtyActorReferences.delete(index);
    syncActorLibraryUploadControl(index);
    renderActorAssetPreview(index);
    renderActorReferencePreviews(state.lastJob?.actors || []);
    updateCostAndButton();
  };
  $(`[data-actor-index="${index}"]`).querySelectorAll("[data-actor-source-mode]").forEach((button) => {
    button.addEventListener("click", () => {
      sourceSelect.value = button.dataset.actorSourceMode;
      syncSource();
    });
  });
  $(`#actorAsset${index}`).addEventListener("change", () => {
    if ($(`#actorAsset${index}`).value) $(`#actorAssetManual${index}`).value = "";
    renderActorAssetPreview(index); renderActorReferencePreviews(state.lastJob?.actors || []); updateCostAndButton();
  });
  $(`#actorAssetManual${index}`).addEventListener("input", () => {
    if ($(`#actorAssetManual${index}`).value.trim()) $(`#actorAsset${index}`).value = "";
    renderActorAssetPreview(index); renderActorReferencePreviews(state.lastJob?.actors || []); updateCostAndButton();
  });
  $(`#actorArkName${index}`).addEventListener("input", (event) => { event.currentTarget.dataset.autoName = "0"; });
  $(`[data-actor-index="${index}"] [data-create-character-group]`)?.addEventListener("click", (event) => {
    openQuickCharacterGroupDialog(event.currentTarget.dataset.createCharacterGroup);
  });
  $(`[data-upload-actor-character="${index}"]`)?.addEventListener("click", () => uploadSingleActorToLibrary(index));
  $(`[data-delete-actor-character="${index}"]`)?.addEventListener("click", () => deleteActorCharacterAsset(index));
  syncActorLibraryUploadControl(index);
}

function renderSingleActor(index, existing = null) {
  const current = document.querySelector(`[data-actor-index="${index}"]`);
  if (!current) return;
  current.outerHTML = actorTemplate(index, existing);
  bindActor(index);
  updateActorControls();
}

function renderActorList(existingActors = []) {
  $("#actorList").innerHTML = "";
  for (let index = 1; index <= state.actorCount; index += 1) {
    const existing = existingActors.find((actor) => Number(actor.id) === index) || null;
    $("#actorList").insertAdjacentHTML("beforeend", actorTemplate(index, existing));
    bindActor(index);
  }
  updateActorControls();
}

function updateActorControls() {
  $("#actorCountLabel").textContent = `${state.actorCount} 位角色`;
  $("#addActorBtn").disabled = state.actorCount >= 2 || state.lastJob?.status === "running";
  $("#removeActorBtn").disabled = state.actorCount <= 1 || state.lastJob?.status === "running";
}

function addActor() {
  if (state.actorCount >= 2) return toast("真实人物角色库模式当前最多支持2位人物。", true);
  state.actorCount += 1;
  $("#actorList").insertAdjacentHTML("beforeend", actorTemplate(state.actorCount, existingActor(state.actorCount)));
  bindActor(state.actorCount);
  updateActorControls();
  applyAutomaticCastSuggestions();
  renderShotGrid();
  invalidateCastConfirmation();
}

function removeActor() {
  if (state.actorCount <= 1) return;
  const removed = state.actorCount;
  $(`[data-actor-index="${removed}"]`)?.remove();
  state.actorCount -= 1;
  for (const [shotIndex, actorIds] of state.shotCasts) {
    state.shotCasts.set(shotIndex, actorIds.filter((value) => value !== removed));
  }
  updateActorControls();
  renderShotGrid();
  invalidateCastConfirmation();
}

function stableDetectorPeopleCount(shot) {
  const counts = (shot?.sample_actor_counts || []).map(Number).filter((value) => Number.isFinite(value) && value >= 0 && value <= 4);
  if (counts.length) {
    const repeated = [...new Set(counts.filter((value) => counts.filter((item) => item === value).length >= 2))].sort((a, b) => a - b);
    if (repeated.length) return repeated[repeated.length - 1];
    return [...counts].sort((a, b) => a - b)[Math.floor(counts.length / 2)];
  }
  return Math.max(Number(shot?.suggested_actor_count || 0), Array.isArray(shot?.person_slots) ? shot.person_slots.length : 0);
}

function detectedPeopleCount(shot) {
  const performanceSlots = [
    ...(shot?.performance?.performance || []).map((item) => Number(item.actor_slot || 0)),
    ...(shot?.performance?.dialogue || []).map((item) => Number(item.speaker_slot || 0)),
  ].filter((value) => Number.isFinite(value) && value > 0);
  const performanceCount = performanceSlots.length ? Math.max(...performanceSlots) : 0;
  return Math.max(0, performanceCount, stableDetectorPeopleCount(shot));
}

function performanceSlotAnchor(shot, slot) {
  if (shot?.position_binding_manual) {
    const mapping = (shot.actor_mappings || []).find((item) => Number(item.slot) === Number(slot));
    if (mapping?.source_position) return String(mapping.source_position);
  }
  const semanticSlots = [...new Set((shot?.performance?.performance || []).map((item) => Number(item.actor_slot || 0)).filter((value) => value > 0))];
  const detectorCount = stableDetectorPeopleCount(shot);
  const performanceIsComplete = semanticSlots.length >= detectorCount;
  const performances = performanceIsComplete
    ? (shot?.performance?.performance || []).filter((item) => Number(item.actor_slot) === Number(slot))
    : [];
  const evidence = performances.map((item) => String(item.visible_evidence || "").trim()).filter(Boolean).slice(0, 2).join("；");
  const intent = performances.map((item) => String(item.core_intent || "").trim()).filter(Boolean).slice(0, 2).join("；");
  if (evidence || intent) return evidence || intent;
  const detected = (shot?.person_slots || [])[Number(slot) - 1] || {};
  return detected.position || `表演槽位 P${slot}`;
}

function positionLockValue(shot, slot) {
  const key = `${Number(shot.index)}:${Number(slot)}`;
  return state.positionDrafts.has(key) ? state.positionDrafts.get(key) : performanceSlotAnchor(shot, slot);
}

function continuityCharacterFor(shot, slot) {
  const assignments = state.lastJob?.cast_continuity?.assignments || [];
  const match = assignments.find((item) => Number(item.shot_index) === Number(shot.index) && Number(item.slot) === Number(slot));
  return match ? Number(match.character_id || 0) : 0;
}

function nextSceneGroupId() {
  let candidate;
  do {
    candidate = `scene_${state.nextSceneGroupId}`;
    state.nextSceneGroupId += 1;
  } while (state.sceneGroups.some((group) => group.id === candidate));
  return candidate;
}

function normalizeSceneGroup(group, position) {
  const id = group?.id || nextSceneGroupId();
  const images = (group?.images || []).map((image, offset) => ({
    index: Number(image.index || offset + 1),
    name: image.name || `场景图 ${offset + 1}`,
    url: image.url || "",
    file: image.file || null,
  }));
  return { id, name: group?.name || `新场景 ${position}`, description: group?.description || "", images };
}

function sceneGroupById(groupId) {
  return state.sceneGroups.find((group) => group.id === groupId) || null;
}

function sceneAssignmentFor(index) {
  return state.shotScenes.get(Number(index)) || null;
}

function sceneGroupTemplate(group, position) {
  const imageItems = group.images.map((image) => `
    <div class="scene-library-image">
      ${image.url || image.file ? `<img src="${image.file ? URL.createObjectURL(image.file) : image.url}" alt="${escapeHtml(image.name)}">` : ""}
      <span><b>A${position}-${image.index}</b><small>${escapeHtml(image.name)}</small></span>
    </div>`).join("");
  return `<article class="scene-group-card" data-scene-group="${group.id}">
    <div class="scene-group-head"><span>${String(position).padStart(2, "0")}</span><div><input data-scene-name="${group.id}" maxlength="60" value="${escapeHtml(group.name)}" aria-label="场景组名称"><small>同一地点可上传1–6个角度</small></div><button type="button" data-remove-scene-group="${group.id}" ${state.sceneGroups.length <= 1 ? "disabled" : ""}>删除</button></div>
    <textarea data-scene-description="${group.id}" rows="2" maxlength="240" placeholder="可选：例如现代客厅、暖色木质、夜景">${escapeHtml(group.description)}</textarea>
    <div class="scene-library-images">${imageItems || `<div class="scene-library-empty">尚未上传场景图片</div>`}</div>
    <label class="mini-drop scene-multi-drop" for="sceneFiles${group.id}"><input id="sceneFiles${group.id}" data-scene-files="${group.id}" type="file" accept="image/*" multiple><span>选择该场景的多角度图片</span><small>${group.images.length ? `已准备 ${group.images.length} 张，可再次选择进行替换` : "最多6张；系统会为每个分镜选择具体角度"}</small></label>
  </article>`;
}

function renderSceneLibrary() {
  const container = $("#sceneGroupList");
  if (!container) return;
  container.innerHTML = state.sceneGroups.map((group, offset) => sceneGroupTemplate(group, offset + 1)).join("");
  $("#sceneGroupCount").textContent = `${state.sceneGroups.length} 个场景组 · ${state.sceneGroups.reduce((sum, group) => sum + group.images.length, 0)} 张图`;
  renderWholeSceneOptions();
}

function initializeSceneLibrary(job, preserveLocal = false) {
  const savedGroups = job?.scene_groups || [];
  if (!preserveLocal || savedGroups.length) {
    state.sceneGroups = savedGroups.map((group, offset) => normalizeSceneGroup(group, offset + 1));
  }
  state.nextSceneGroupId = 1;
  for (const group of state.sceneGroups) {
    const match = /^scene_(\d+)$/.exec(group.id);
    if (match) state.nextSceneGroupId = Math.max(state.nextSceneGroupId, Number(match[1]) + 1);
  }
  if (!state.sceneGroups.length) state.sceneGroups.push(normalizeSceneGroup(null, 1));
  state.shotScenes.clear(); state.manualSceneShots.clear();
  for (const shot of job?.shots || []) {
    if (shot.scene_group_id && Number(shot.scene_image_index) > 0 && sceneGroupById(shot.scene_group_id)) {
      state.shotScenes.set(Number(shot.index), { groupId: shot.scene_group_id, imageIndex: Number(shot.scene_image_index) });
    }
  }
  renderSceneLibrary();
}

function autoMatchScenes(showToast = true) {
  const groups = state.sceneGroups.filter((group) => group.images.length);
  const shots = state.lastJob?.shots || [];
  if (!shots.length) return showToast && toast("请先完成分镜分析。", true);
  if (!groups.length) return showToast && toast("请先在场景库上传至少一组新场景图。", true);
  const usage = new Map(groups.map((group) => [group.id, 0]));
  const clusters = [...new Set(shots.map((shot) => Number(shot.scene_cluster || shot.index)))];
  const groupForCluster = new Map(clusters.map((cluster, offset) => {
    const groupIndex = groups.length === 1 ? 0 : Math.min(groups.length - 1, Math.floor(offset * groups.length / clusters.length));
    return [cluster, groups[groupIndex]];
  }));
  shots.forEach((shot) => {
    const cluster = Number(shot.scene_cluster || shot.index);
    const group = groupForCluster.get(cluster) || groups[0];
    const used = usage.get(group.id) || 0;
    state.shotScenes.set(Number(shot.index), { groupId: group.id, imageIndex: used % group.images.length + 1 });
    usage.set(group.id, used + 1);
  });
  state.manualSceneShots.clear();
  renderShotGrid(); invalidateCastConfirmation();
  if (showToast) toast(`已按 ${clusters.length} 个原片场景组匹配 ${shots.length} 个镜头，请核对每镜场景和角度。`);
}

function sceneMappingsComplete() {
  return (state.lastJob?.shots || []).every((shot) => {
    if (shot.generation_skipped === true) return true;
    const assignment = sceneAssignmentFor(shot.index);
    const group = assignment ? sceneGroupById(assignment.groupId) : null;
    return Boolean(group && group.images.some((image) => image.index === Number(assignment.imageIndex)));
  });
}

function applyAutomaticCastSuggestions() {
  const continuityMap = new Map();
  for (const shot of state.lastJob?.shots || []) {
    const savedMappings = Array.isArray(shot.actor_mappings)
      ? [...shot.actor_mappings].sort((a, b) => Number(a.slot) - Number(b.slot))
      : [];
    for (const mapping of savedMappings) {
      const characterId = continuityCharacterFor(shot, Number(mapping.slot));
      if (characterId && !continuityMap.has(characterId)) continuityMap.set(characterId, Number(mapping.actor_id));
    }
  }
  const usedActors = new Set([...continuityMap.values()].filter((actorId) => actorId > 0 && actorId <= state.actorCount));
  const continuityAssignments = [...(state.lastJob?.cast_continuity?.assignments || [])]
    .sort((a, b) => Number(a.shot_index) - Number(b.shot_index) || Number(a.slot) - Number(b.slot));
  for (const assignment of continuityAssignments) {
    const characterId = Number(assignment.character_id || 0);
    if (!characterId || continuityMap.has(characterId)) continue;
    const nextActorId = Array.from({ length: state.actorCount }, (_value, offset) => offset + 1)
      .find((actorId) => !usedActors.has(actorId));
    if (!nextActorId) continue;
    continuityMap.set(characterId, nextActorId);
    usedActors.add(nextActorId);
  }
  for (const shot of state.lastJob?.shots || []) {
    const index = Number(shot.index);
    if (state.manualCasts.has(index)) continue;
    if (shot.cast_confirmed === true) {
      const savedMappings = Array.isArray(shot.actor_mappings)
        ? [...shot.actor_mappings].sort((a, b) => Number(a.slot) - Number(b.slot)).map((value) => Number(value.actor_id))
        : (shot.actor_ids || []).map(Number);
      const validSavedMappings = savedMappings.filter((value) => value > 0 && value <= state.actorCount);
      if (validSavedMappings.length >= detectedPeopleCount(shot)) {
        state.shotCasts.set(index, validSavedMappings);
        continue;
      }
    }
    const continuitySuggested = Array.from({ length: detectedPeopleCount(shot) }, (_value, offset) => {
      const characterId = continuityCharacterFor(shot, offset + 1);
      return continuityMap.get(characterId) || 0;
    });
    if (continuitySuggested.length && continuitySuggested.every((value) => value > 0 && value <= state.actorCount)) {
      state.shotCasts.set(index, continuitySuggested);
      continue;
    }
    const detected = detectedPeopleCount(shot);
    if (state.lastJob?.cast_continuity?.manual_review_required === true) {
      // Missing identities must be reviewed, not silently assigned by screen order.
      state.shotCasts.set(index, Array.from({ length: detected }, () => 0));
      continue;
    }
    const suggested = Math.min(state.actorCount, Math.max(0, detected));
    state.shotCasts.set(index, Array.from({ length: suggested }, (_value, offset) => offset + 1));
  }
}

function castFor(index) { return state.shotCasts.get(Number(index)) || []; }

function actorRole(index) {
  return $(`#roleDescription${index}`)?.value.trim() || existingActor(index)?.role || `人物${index}`;
}

function routeText(actorIds) {
  if (actorIds.some((id) => !id)) return "待核对：请为每个原片人物选择对应的新人物";
  if (!actorIds.length) return "无人镜头：使用深度视频和新场景板进行场景重绘";
  if (actorIds.length === 1) return `单人路由：使用人物 ${actorIds[0]}，调用项目1生成流程`;
  return `多人路由：使用人物 ${actorIds.join("、")}，调用项目2多人生成流程`;
}

function confidenceLabel(value) {
  return value === "high" ? "较高" : value === "medium" ? "中等" : "较低，请重点核对";
}

function extractedDialogueText(performance) {
  return (performance?.dialogue || []).map((item) => {
    const delivery = item.delivery ? `；语气：${item.delivery}` : "";
    const mouth = item.mouth_motion ? `；口型：${item.mouth_motion}` : "";
    const source = item.source_type || "character";
    const sourceLabel = source === "character" ? `P${Number(item.speaker_slot || 1)}` : source === "offscreen" ? "画外声" : source === "bgm_vocal" ? "BGM人声（不生成）" : source === "ignore" ? "忽略声音" : "来源待确认";
    return `[${Number(item.start || 0).toFixed(2)}–${Number(item.end || 0).toFixed(2)}秒] ${sourceLabel}：${item.text || ""}${delivery}${mouth}`;
  }).join("\n");
}

function extractedPerformanceText(performance) {
  return (performance?.performance || []).map((item) => {
    const evidence = item.visible_evidence ? `；可见证据：${item.visible_evidence}` : "";
    const exclude = item.exclude ? `；避免误读：${item.exclude}` : "";
    return `[${Number(item.start || 0).toFixed(2)}–${Number(item.end || 0).toFixed(2)}秒] P${Number(item.actor_slot || 1)}：${item.core_intent || ""}${evidence}${exclude}`;
  }).join("\n");
}

function performanceEditorValue(shot, field) {
  const index = Number(shot.index);
  const draft = state.performanceDrafts.get(index);
  if (draft && Object.prototype.hasOwnProperty.call(draft, field)) return draft[field];
  const performance = shot.performance || {};
  const manualField = field === "dialogue" ? "manual_dialogue_text" : "manual_performance_text";
  if (Object.prototype.hasOwnProperty.call(performance, manualField)) return String(performance[manualField] || "");
  return field === "dialogue" ? extractedDialogueText(performance) : extractedPerformanceText(performance);
}

function dialogueSourceEditor(shot, disabled) {
  const dialogue = shot?.performance?.dialogue || [];
  if (!dialogue.length) return `<div class="dialogue-source-empty">本镜未提取到语言声；如有遗漏，可直接在下方台词框补写。</div>`;
  const people = Math.max(1, Math.min(4, detectedPeopleCount(shot) || 1));
  return `<div class="dialogue-source-editor"><div class="dialogue-source-title"><b>逐句声音来源与说话人</b><small>嘴部不可见、人物背对或被遮挡，不代表不是说话人；请按剧情和人物位置锁定。</small></div>${dialogue.map((item, itemIndex) => {
    const sourceType = item.source_type || "character";
    const current = sourceType === "character" ? `p${Number(item.speaker_slot || 1)}` : sourceType;
    const characterOptions = Array.from({ length: people }, (_value, offset) => offset + 1).map((slot) =>
      `<option value="p${slot}" ${current === `p${slot}` ? "selected" : ""}>画内人物 P${slot}</option>`
    ).join("");
    const options = `${characterOptions}<option value="offscreen" ${current === "offscreen" ? "selected" : ""}>画外人物台词</option><option value="bgm_vocal" ${current === "bgm_vocal" ? "selected" : ""}>BGM 歌声/人声（不生成）</option><option value="ignore" ${current === "ignore" ? "selected" : ""}>其他声音（忽略）</option><option value="uncertain" ${current === "uncertain" ? "selected" : ""}>来源待确认（禁止生成）</option>`;
    return `<div class="dialogue-source-row"><span><b>${Number(item.start || 0).toFixed(2)}–${Number(item.end || 0).toFixed(2)}秒</b><small>${escapeHtml(item.text || "（未识别文字）")}</small></span><select data-dialogue-source-index="${itemIndex}" ${disabled}>${options}</select></div>`;
  }).join("")}</div>`;
}

function performanceEditor(shot, disabled) {
  if (!shot.has_performance) return "";
  const index = Number(shot.index);
  const performance = shot.performance || {};
  const manuallyReviewed = performance.manual_reviewed === true;
  const needsRegeneration = shot.performance_dirty === true;
  return `<section class="performance-editor ${needsRegeneration ? "dirty" : ""}">
    <div class="performance-editor-head"><div><b>台词与表演人工校订</b><small>人工文字保存后优先级最高，会覆盖本镜 AI 提取结果</small></div><span>${needsRegeneration ? "已修改 · 待重生" : manuallyReviewed ? "已人工保存" : "待人工核对"}</span></div>
    ${dialogueSourceEditor(shot, disabled)}
    <label><span>台词、说话人、语气与口型时序</span><textarea data-performance-dialogue="${index}" rows="4" maxlength="1800" placeholder="无台词时留空；有台词请保留时间段和 P1/P2 说话人" ${disabled}>${escapeHtml(performanceEditorValue(shot, "dialogue"))}</textarea></label>
    <label><span>表演、情绪、眼神、表情与姿态</span><textarea data-performance-acting="${index}" rows="4" maxlength="1800" placeholder="按时间段描述 P1/P2 的完整表演语义与可见证据" ${disabled}>${escapeHtml(performanceEditorValue(shot, "performance"))}</textarea></label>
    <div class="performance-editor-actions"><button type="button" data-reset-performance="${index}" ${disabled}>恢复 AI 提取文字</button><button class="save-performance" type="button" data-save-performance="${index}" ${disabled}>保存本镜校订</button></div>
  </section>`;
}

function renderFinalStyleLibrary(selectedId = REAL_DEFAULT_FINAL_STYLE) {
  const presets = state.config?.final_style_presets || [];
  const selected = presets.some((preset) => preset.id === selectedId) ? selectedId : REAL_DEFAULT_FINAL_STYLE;
  $("#finalStyle").value = selected;
  $("#finalStyleLibrary").innerHTML = presets.map((preset) => `
    <button type="button" class="style-preset-card ${preset.id === selected ? "active" : ""}" data-final-style="${escapeHtml(preset.id)}">
      <b>${escapeHtml(preset.label)}</b><small>${escapeHtml(preset.description)}</small>
    </button>`).join("");
}

function hasHardWhiteTextViolation(shot) {
  const reasons = (shot?.white_model_qa?.reasons || []).join("；");
  return ["字幕", "文字", "时间码", "Logo", "水印"].some((token) => reasons.includes(token));
}

function shotCard(shot, job) {
  const index = Number(shot.index);
  const generationSkipped = shot.generation_skipped === true;
  const actorIds = castFor(index);
  const media = shot.has_output ? shot.output_url : shot.has_white_model ? shot.white_model_url : shot.has_mosaic ? shot.mosaic_url : shot.has_depth ? shot.depth_url : shot.source_url;
  const mediaLabel = generationSkipped
    ? (shot.has_output ? "已跳过（保留旧成片预览）" : "已跳过")
    : shot.has_output ? "最终成片" : shot.has_white_model ? "白模绿幕" : shot.has_mosaic ? "人脸打码" : shot.has_depth ? (shot.depth_padded ? "深度视频（片尾已定格延长）" : "深度视频") : "原始分镜";
  const mediaRevision = shot.task_id || shot.white_model_task_id || shot.white_model_signature || shot.output_signature || job.created_at;
  const mediaSrc = media ? `${media}?preview=${encodeURIComponent(`${job.id}-${index}-${mediaRevision}`)}` : "";
  const disabled = job.status === "running" && job.kind === "long_generate" ? "disabled" : "";
  const personSlots = Array.isArray(shot.person_slots) ? shot.person_slots : [];
  const mappingRows = actorIds.map((actorId, offset) => {
    const detected = personSlots[offset] || {};
    const position = performanceSlotAnchor(shot, offset + 1);
    const characterId = continuityCharacterFor(shot, offset + 1);
    const modelColor = ({red:"红",white:"白",blue:"蓝",yellow:"黄"})[shot.white_color_plan?.find(r => r.id === `p${offset+1}`)?.color] || "";
    const hasSemanticSlot = (shot?.performance?.performance || []).some((item) => Number(item.actor_slot) === offset + 1);
    const confidence = hasSemanticSlot
      ? " · 来自台词表演分析"
      : detected.confidence ? ` · 检测置信度${Math.round(Number(detected.confidence) * 100)}%` : " · 人工添加";
    const options = `<option value="0" ${!actorId ? "selected" : ""}>请选择对应人物</option>` + Array.from({ length: state.actorCount }, (_value, actorOffset) => actorOffset + 1).map((optionId) =>
      `<option value="${optionId}" ${optionId === actorId ? "selected" : ""}>人物${optionId} · ${escapeHtml(actorRole(optionId))}</option>`
    ).join("");
    const continuityNote = characterId ? " · C身份仅供自动建议，人工选择只作用于本镜" : "";
    return `<div class="person-map-row"><span><b>表演槽位 P${offset + 1}${modelColor ? ` · ${modelColor}模` : ""}${characterId ? ` · 原片身份 C${characterId}` : ""}</b><small>${escapeHtml(position)}${confidence}${continuityNote}</small></span><i>替换为</i><select data-person-mapping="${index}" data-person-slot="${offset + 1}" ${disabled}>${options}</select><label class="position-lock-field"><span>位置与动作锁定</span><input data-position-lock="${index}" data-position-slot="${offset + 1}" maxlength="220" value="${escapeHtml(positionLockValue(shot, offset + 1))}" placeholder="例如：左侧内景坐着，较小；右侧前景站着，较大" ${disabled}></label></div>`;
  }).join("");
  const assignment = sceneAssignmentFor(index);
  const selectedGroup = assignment ? sceneGroupById(assignment.groupId) : null;
  const selectedImage = selectedGroup?.images.find((image) => image.index === Number(assignment?.imageIndex)) || null;
  const groupOptions = [`<option value="">选择新场景组</option>`, ...state.sceneGroups.map((group) =>
    `<option value="${group.id}" ${group.id === assignment?.groupId ? "selected" : ""} ${group.images.length ? "" : "disabled"}>${escapeHtml(group.name)} · ${group.images.length}张</option>`
  )].join("");
  const imageOptions = selectedGroup ? selectedGroup.images.map((image) =>
    `<option value="${image.index}" ${image.index === Number(assignment?.imageIndex) ? "selected" : ""}>角度 ${image.index} · ${escapeHtml(image.name)}</option>`
  ).join("") : "";
  const scenePreview = selectedImage ? (selectedImage.file ? URL.createObjectURL(selectedImage.file) : selectedImage.url) : "";
  const sceneStatus = selectedImage ? `${selectedGroup.name} · 角度${selectedImage.index}` : "尚未匹配新场景";
  const actions = [[shot.source_url, "原始", "video"], [shot.people_map_url, "人物标注", "image"], [shot.mosaic_url, "打码", "video"], [shot.white_model_url, "白模", "video"], [shot.performance_url, "表演JSON", "json"], [shot.depth_url, "深度", "video"], [shot.target_scene_url, "替换场景", "image"], [shot.scene_url, "生成场景参考", "image"], [shot.output_url, "成片", "video"]]
    .filter(([url]) => url)
    .map(([url, label, type]) => `<a class="shot-download" href="${url}?download=1" data-url="${url}" data-name="分镜${String(index).padStart(2, "0")}_${label}.${type === "image" ? "jpg" : type === "json" ? "json" : "mp4"}" data-type="${type}">下载${label}</a>`).join("");
  const performance = shot.performance || {};
  const performanceSummary = shot.has_performance ? `<div class="performance-summary">已提取 ${Number(performance.dialogue?.length || 0)} 句台词 · ${Number(performance.performance?.length || 0)} 段表演语义${performance.audio_summary ? ` · ${escapeHtml(performance.audio_summary)}` : ""}</div>` : "";
  const performanceForm = performanceEditor(shot, disabled);
  const whiteQa = shot.white_model_qa || {};
  const hardWhiteTextFailure = hasHardWhiteTextViolation(shot);
  const whiteQaNotice = generationSkipped || !shot.has_white_model
    ? ""
    : whiteQa.passed === false
      ? `<div class="scene-plate-note">白模质量闸门未通过：${escapeHtml((whiteQa.reasons || []).join("；"))}。${hardWhiteTextFailure ? "字幕和可读文字属于硬性违规，不能人工放行；请重新生成此镜白模。" : shot.white_model_manually_approved ? "本镜已由人工确认可用，最终成片仍会再次验收。" : "该白模不会提交最终成片；请重新生成、人工确认或跳过。"}</div>`
      : whiteQa.passed === true
        ? `<div class="scene-plate-note">白模质量闸门已通过${(whiteQa.warnings || []).length ? `；允许偏差：${escapeHtml(whiteQa.warnings.join("；"))}` : "：人物占位、绿幕、材质与文字检测无致命问题"}。</div>`
        : `<div class="scene-plate-note">旧白模尚未执行新版质量闸门；提交成片前系统会先本地验收。</div>`;
  const compositionQa = shot.composition_qa || {};
  const compositionQaNotice = generationSkipped
    ? `<div class="scene-plate-note">本分镜已由人工跳过，不会继续产生付费任务，也不会进入最终整片；旧结果仍保留供预览和下载。需要重新制作时可点击“恢复此镜”。</div>`
    : compositionQa.passed === false
    ? `<div class="scene-plate-note">构图验收未通过：${escapeHtml((compositionQa.reasons || []).join("；"))}。${shot.composition_approval_required ? `已完成 ${Number(shot.composition_retry_count || 0)} 次纠偏，系统已停止继续付费生成，需人工同意后才能重试。` : "系统正在按限制自动纠偏。"}</div>`
    : compositionQa.passed === true ? `<div class="scene-plate-note">自动构图/景别/运镜检查未发现明显差异。</div>` : "";
  const comparisonItems = [[shot.source_url, "原片"], [shot.white_model_url, "白膜"], [shot.output_url, "成片"]]
    .filter(([url]) => url)
    .map(([url, label]) => ({ url, label }));
  if (media && !comparisonItems.some((item) => item.url === media)) comparisonItems.push({ url: media, label: mediaLabel });
  const comparisonTabs = comparisonItems.length > 1 ? `<div class="shot-compare-tabs" aria-label="原片、白膜与成片切换">${comparisonItems.map((item) => {
    const source = `${item.url}?preview=${encodeURIComponent(`${job.id}-${index}-${item.label}-${mediaRevision}`)}`;
    return `<button type="button" class="${item.url === media ? "active" : ""}" data-shot-compare-src="${escapeHtml(source)}" data-shot-compare-label="${escapeHtml(item.label)}">${escapeHtml(item.label)}</button>`;
  }).join("")}</div>` : "";
  const preview = media ? `<div class="shot-comparison">${comparisonTabs}<div class="shot-preview-shell"><video class="shot-preview" controls playsinline preload="none" data-media-src="${escapeHtml(mediaSrc)}"></video><button type="button" class="shot-preview-load" data-load-shot-preview="${index}">加载${mediaLabel}预览</button></div><small class="shot-preview-caption">当前预览：${escapeHtml(mediaLabel)}</small></div>` : "";
  const semanticCount = detectedPeopleCount(shot);
  const detectorCount = stableDetectorPeopleCount(shot);
  const performanceCount = Math.max(0, ...(performance.performance || []).map((item) => Number(item.actor_slot || 0)));
  const countWarning = shot.has_performance && performanceCount < detectorCount ? ` · 表演分析漏记${detectorCount - performanceCount}人，已按本地检测补齐` : "";
  return `<article class="shot-card" data-shot-index="${index}"><div class="shot-head"><div><b>分镜 ${String(index).padStart(2, "0")}</b><small>${Number(shot.start).toFixed(2)}s–${Number(shot.end).toFixed(2)}s · ${Number(shot.duration).toFixed(2)}秒 · 原场景组 S${Number(shot.scene_cluster || index)} · ${mediaLabel}</small></div><span class="shot-status ${shot.status || "ready"}">${escapeHtml(shot.stage || shot.status || "ready")}</span></div>${preview}${shot.has_people_map ? `<div class="people-map-preview"><img src="${shot.people_map_url}?preview=${job.id}-${index}-people" alt="分镜人物检测标注"><small>彩框只是本地检测参考；多人重叠时可能重复。下方最终映射按“表演槽位”及动作位置锁定。</small></div>` : ""}${performanceSummary}${performanceForm}<div class="shot-cast"><div class="shot-cast-head"><b>原片表演槽位 → 新人物形象</b><span class="detection-badge ${shot.detection_confidence || "low"}">表演槽位 ${semanticCount} 人${escapeHtml(countWarning)}</span></div><div class="person-map-list">${mappingRows || `<div class="empty-person-map">当前标记为无人镜头；识别遗漏时点击“增加原片人物”。</div>`}</div><div class="person-map-controls"><button type="button" data-add-person="${index}" ${disabled}>＋ 增加原片人物</button><button type="button" data-remove-person="${index}" ${!actorIds.length || disabled ? "disabled" : ""}>减少一人</button><button class="clear-cast ${actorIds.length ? "" : "active"}" type="button" data-clear-cast="${index}" ${disabled}>无人镜头</button></div><small class="shot-route">${routeText(actorIds)}；下拉选择只修改当前分镜槽位，不会联动其他分镜；人工选择高于自动C身份建议。</small></div><div class="shot-scene-map ${selectedImage ? "matched" : "missing"}"><div class="shot-scene-map-head"><div><b>本镜头新场景</b><small>${escapeHtml(sceneStatus)}</small></div><span>${selectedImage ? "已匹配" : "必须选择"}</span></div><div class="shot-scene-selects"><select data-shot-scene-group="${index}" ${disabled}>${groupOptions}</select><select data-shot-scene-image="${index}" ${!selectedGroup || disabled ? "disabled" : ""}>${imageOptions || `<option value="">先选择场景组</option>`}</select></div>${scenePreview ? `<img class="shot-scene" src="${scenePreview}" alt="本镜匹配的新场景">` : shot.has_target_scene ? `<img class="shot-scene" src="${shot.target_scene_url}?preview=${job.id}-${index}-target-scene" alt="已保存的新场景">` : ""}${shot.has_scene ? `<div class="scene-plate-note">已生成匹配机位的新场景板，可在下方下载预览。</div>` : ""}</div>${whiteQaNotice}${compositionQaNotice}<div class="shot-progress"><span style="width:${Math.max(0, Math.min(100, shot.progress || 0))}%"></span></div><div class="shot-artifact-row">${actions}</div>${shot.error ? `<div class="shot-error">${escapeHtml(shot.error)}</div>` : ""}</article>`;
}

function activateShotPreview(videoNode, onSettled = () => {}, forceReload = false) {
  if (!videoNode) { onSettled(); return; }
  const source = videoNode.dataset.mediaSrc;
  const shell = videoNode.closest(".shot-preview-shell");
  const button = shell?.querySelector(".shot-preview-load");
  if (!source) { onSettled(); return; }
  if (!forceReload && videoNode.dataset.loadedSource === source && videoNode.readyState >= 1) {
    if (button) button.hidden = true;
    onSettled(); return;
  }
  if (button) { button.hidden = false; button.disabled = true; button.textContent = "正在加载预览…"; }
  let finished = false;
  const finish = (succeeded) => {
    if (finished) return;
    finished = true;
    clearTimeout(videoNode._previewTimer);
    if (button) {
      button.disabled = false;
      button.hidden = succeeded;
      button.textContent = succeeded ? "已加载" : "重新加载预览";
    }
    if (!succeeded) videoNode.dataset.loadedSource = "";
    onSettled();
  };
  videoNode.addEventListener("loadedmetadata", () => finish(true), { once: true });
  videoNode.addEventListener("error", () => finish(false), { once: true });
  videoNode.dataset.loadedSource = source;
  videoNode.src = forceReload ? `${source}&retry=${Date.now()}` : source;
  videoNode.load();
  videoNode._previewTimer = setTimeout(() => finish(videoNode.readyState >= 1), 5000);
}

function hydrateShotPreviews() {
  const token = ++shotPreviewLoadToken;
  const previews = [...document.querySelectorAll("#shotGrid .shot-preview")];
  let index = 0;
  const loadNext = () => {
    if (token !== shotPreviewLoadToken || index >= previews.length) return;
    activateShotPreview(previews[index++], loadNext);
  };
  loadNext();
}

function renderShotGrid() {
  if (!state.lastJob) return;
  $("#shotGrid").innerHTML = (state.lastJob.shots || []).map((shot) => shotCard(shot, state.lastJob)).join("");
  const running = ["queued", "running"].includes(state.lastJob.status);
  for (const shot of state.lastJob.shots || []) {
    const card = $(`[data-shot-index="${Number(shot.index)}"]`);
    if (!card) continue;
    const status = card.querySelector(".shot-status");
    if (status) {
      status.insertAdjacentHTML(
        "afterend",
        `<button type="button" class="shot-delete-button" data-delete-shot="${Number(shot.index)}" ${running ? "disabled" : ""}>删除此镜</button>`,
      );
    }
    if (shot.has_output && !shot.composition_approval_required && shot.generation_skipped !== true) {
      const actionRow = card.querySelector(".shot-artifact-row");
      if (actionRow) {
        actionRow.insertAdjacentHTML(
          "beforeend",
          `<button type="button" class="shot-regenerate-button" data-regenerate-shot="${Number(shot.index)}" ${running ? "disabled" : ""}>重新生成此镜</button>`,
        );
      }
    }
    if (shot.composition_approval_required && shot.generation_skipped !== true) {
      const actionRow = card.querySelector(".shot-artifact-row");
      if (actionRow) {
        actionRow.insertAdjacentHTML(
          "beforeend",
          `<button type="button" class="shot-regenerate-button" data-approve-composition-retry="${Number(shot.index)}" ${running ? "disabled" : ""}>同意继续纠偏生成</button>`,
        );
      }
    }
    if (shot.white_model_approval_required && shot.generation_skipped !== true && !hasHardWhiteTextViolation(shot)) {
      const actionRow = card.querySelector(".shot-artifact-row");
      if (actionRow) {
        actionRow.insertAdjacentHTML(
          "beforeend",
          `<button type="button" class="shot-regenerate-button" data-approve-white-model="${Number(shot.index)}" ${running ? "disabled" : ""}>人工确认使用此白模</button>`,
        );
      }
    }
    const canSkipFinalShot = shot.generation_skipped !== true
      && (shot.has_output || shot.composition_approval_required || shot.white_model_approval_required || shot.status === "failed");
    if (canSkipFinalShot) {
      const actionRow = card.querySelector(".shot-artifact-row");
      if (actionRow) {
        actionRow.insertAdjacentHTML(
          "beforeend",
          `<button type="button" class="shot-delete-button" data-skip-shot="${Number(shot.index)}" ${running ? "disabled" : ""}>跳过此镜并继续</button>`,
        );
      }
    }
    if (shot.generation_skipped === true) {
      const actionRow = card.querySelector(".shot-artifact-row");
      if (actionRow) {
        actionRow.insertAdjacentHTML(
          "beforeend",
          `<button type="button" class="shot-regenerate-button" data-restore-skipped-shot="${Number(shot.index)}" ${running ? "disabled" : ""}>恢复并重新生成此镜</button>`,
        );
      }
    }
    const canRetryWhiteModel = shot.has_mosaic && (
      shot.has_white_model
      || shot.status === "failed"
      || shot.status === "retryable"
      || Boolean(shot.error)
    );
    if (canRetryWhiteModel) {
      const actionRow = card.querySelector(".shot-artifact-row");
      if (actionRow) {
        const completedAttempts = Number(shot.white_model_total_generation_count || 0)
          || Math.max(1, Number(shot.white_model_retry_count || 0) + 1);
        const retryLabel = completedAttempts >= 2
          ? "确认费用并重生此镜白模"
          : "重新生成此镜白模";
        actionRow.insertAdjacentHTML(
          "beforeend",
          `<button type="button" class="shot-regenerate-white-button" data-regenerate-white-shot="${Number(shot.index)}" ${running ? "disabled" : ""}>${retryLabel}</button>`,
        );
      }
    }
  }
  hydrateShotPreviews();
}

function invalidateCastConfirmation() {
  state.castConfirmed = false;
  if ($("#confirmCast")) $("#confirmCast").checked = false;
  updateCostAndButton();
}

function refreshShotCast(index) {
  if (!$(`[data-shot-index="${index}"]`)) return;
  renderShotGrid();
}

function updateCostAndButton() {
  const shots = state.lastJob?.shots || [];
  const wholeGeneration = generationStrategy() === WHOLE_GENERATION_STRATEGY;
  const activeShots = shots.filter((shot) => shot.generation_skipped !== true);
  const skippedCount = shots.length - activeShots.length;
  let single = 0; let multi = 0; let empty = 0; let unmatched = 0; let reuse = 0; let seedream = 0; let seedance = 0;
  for (const shot of shots) {
    if (shot.generation_skipped === true) continue;
    const selected = castFor(shot.index);
    const assignment = sceneAssignmentFor(shot.index);
    const sceneGroup = assignment ? sceneGroupById(assignment.groupId) : null;
    const sceneImage = sceneGroup?.images.find((image) => image.index === Number(assignment?.imageIndex));
    if (!sceneImage) unmatched += 1;
    const previous = (shot.actor_ids || []).map(Number);
    const sameCast = previous.length === selected.length && previous.every((value, index) => value === selected[index]);
    const sameScene = Boolean(
      sceneImage && !sceneImage.file && shot.scene_group_id === assignment.groupId
      && Number(shot.scene_image_index) === Number(assignment.imageIndex)
    );
    const sameVoiceMode = shot.dialogue_voice_mode === "seedance_new_voice";
    const sameStyle = shot.final_style_id === $("#finalStyle").value;
    const samePerformance = shot.performance_dirty !== true && shot.position_binding_dirty !== true;
    const previousPositions = [...(shot.actor_mappings || [])].sort((a, b) => Number(a.slot) - Number(b.slot)).map((item) => String(item.source_position || ""));
    const currentPositions = selected.map((_actorId, offset) => positionLockValue(shot, offset + 1));
    const samePositions = previousPositions.length === currentPositions.length && previousPositions.every((value, index) => value === currentPositions[index]);
    if (shot.has_output && sameCast && sameScene && sameVoiceMode && sameStyle && samePerformance && samePositions) { reuse += 1; continue; }
    seedance += 1; seedream += 1;
    if (!selected.length) empty += 1;
    else if (selected.length === 1) single += 1;
    else multi += 1;
  }
  if (shots.length) {
    $("#costTitle").textContent = wholeGeneration
      ? `${activeShots.length} 个分镜 · 整段白模一次生成${skippedCount ? ` · ${skippedCount} 个已跳过` : ""}`
      : `${single} 个单人 · ${multi} 个多人 · ${empty} 个无人重绘 · ${unmatched} 个未匹配${skippedCount ? ` · ${skippedCount} 个已跳过` : ""}`;
    $("#costNote").textContent = wholeGeneration
      ? "预计 0 次 Seedream + 1 次 Seedance 2.5；直接使用所选原始场景图"
      : `预计 ${seedream} 次 Seedream 场景板 + ${seedance} 次 Seedance；${reuse} 个分镜可复用`;
  }
  const running = state.busy || ["queued", "running"].includes(state.lastJob?.status);
  const activelyGenerating = ["queued", "running"].includes(state.lastJob?.status)
    && state.lastJob?.kind === "long_generate";
  const generationPaused = state.lastJob?.status === "paused" && state.lastJob?.kind === "long_generate";
  const pauseButton = $("#pauseGenerateBtn");
  if (pauseButton) {
    pauseButton.classList.toggle("hidden", !activelyGenerating && !generationPaused);
    pauseButton.disabled = !activelyGenerating || state.lastJob?.pause_requested === true;
    pauseButton.textContent = generationPaused ? "已暂停" : state.lastJob?.pause_requested === true ? "正在暂停…" : "暂停生成";
  }
  const whiteReady = activeShots.length > 0 && activeShots.every(whiteModelQualityReady);
  const performanceReady = activeShots.length > 0 && activeShots.every((shot) => Boolean(shot.performance));
  const newVoiceNeedsAnalysis = !performanceReady;
  const usedActorIds = new Set([...state.shotCasts.values()].flat().map(Number));
  const realReferencesReady = Array.from({ length: state.actorCount }, (_value, offset) => offset + 1)
    .every((index) => {
      if (!usedActorIds.has(index)) return true;
      return actorUsesAsset(index, existingActor(index))
        && !state.dirtyActorReferences.has(index)
        && /^asset:\/\/asset-[A-Za-z0-9_-]{6,120}$/.test(actorAssetUri(index, existingActor(index)));
    });
  if (shots.length && newVoiceNeedsAnalysis) {
    $("#costNote").textContent += "；新音色生成前需先完成全部台词与表演分析";
  }
  if (shots.length && !realReferencesReady) $("#costNote").textContent += "；请先把出场人物上传并绑定为 Active 火山角色 Asset";
  const pendingCompositionShots = shots
    .filter((shot) => shot.composition_approval_required && shot.generation_skipped !== true)
    .map((shot) => Number(shot.index));
  const blockedWhiteShots = activeShots.filter((shot) => !whiteModelQualityReady(shot)).map((shot) => Number(shot.index));
  const sceneReady = wholeGeneration ? Boolean($("#wholeSceneReference")?.value) : sceneMappingsComplete();
  const prerequisiteReasons = [];
  if (!state.castConfirmed) prerequisiteReasons.push("请到步骤06勾选“我已核对全部分镜的人物与新场景映射”");
  if (!sceneReady) prerequisiteReasons.push(wholeGeneration ? "请选择整段场景参考图" : "仍有分镜未匹配场景");
  if (!whiteReady) prerequisiteReasons.push("白模尚未全部通过质量闸门或人工确认");
  if (newVoiceNeedsAnalysis) prerequisiteReasons.push("台词与表演分析尚未全部完成");
  if (!realReferencesReady) prerequisiteReasons.push("出场人物尚未全部绑定为 Active 火山角色 Asset");
  const generationBlocked = !activeShots.length || prerequisiteReasons.length > 0 || running;
  const compositionApprovalBlocked = !wholeGeneration && pendingCompositionShots.length > 0;
  const compositionApprovalTitle = compositionApprovalBlocked
    ? `分镜 ${pendingCompositionShots.map((value) => String(value).padStart(2, "0")).join("、")} 等待人工处理；点击主按钮可进入处理流程，也可逐镜重新生成或跳过。`
    : "";
  // Keep the button clickable when only a prerequisite is missing. A disabled grey
  // button gives no feedback; clicking now reports the exact missing item without
  // submitting a paid task. It remains disabled only when there is no task or a
  // task is already running.
  $("#generateBtn").disabled = !activeShots.length || running;
  $("#generateBtn").dataset.blockers = prerequisiteReasons.join("；");
  $("#generateBtn").title = prerequisiteReasons.length
    ? prerequisiteReasons.join("；")
    : compositionApprovalTitle;
  $("#generateBtn").textContent = wholeGeneration
    ? "使用整段白模生成完整成片"
    : pendingCompositionShots.length === 1
    ? `处理分镜 ${String(pendingCompositionShots[0]).padStart(2, "0")} 并继续`
    : "按角色配置生成并合并";
  $("#regenerateAllBtn").disabled = wholeGeneration || generationBlocked || !activeShots.some((shot) => shot.has_output);
  $("#regenerateAllBtn").title = compositionApprovalTitle;
  if (prerequisiteReasons.length) $("#costNote").textContent += `；暂不能生成：${prerequisiteReasons.join("；")}`;
  if (compositionApprovalBlocked) $("#costNote").textContent += `；分镜 ${pendingCompositionShots.map((value) => String(value).padStart(2, "0")).join("、")} 等待人工纠偏同意`;
  if (blockedWhiteShots.length) $("#costNote").textContent += `；分镜 ${blockedWhiteShots.map((value) => String(value).padStart(2, "0")).join("、")} 的白模未通过质量闸门`;
  document.querySelectorAll("[data-regenerate-shot]").forEach((button) => {
    const shot = shots.find((item) => Number(item.index) === Number(button.dataset.regenerateShot));
    button.disabled = wholeGeneration || generationBlocked || !shot?.has_output;
    button.title = wholeGeneration
      ? "整段白模模式只生成完整成片；切回逐分镜模式后可单镜重生。"
      : generationBlocked ? "请先完成本镜所需的角色库 Asset、场景、白模和表演分析。" : "";
  });
  updatePipelineButtons();
}

function updatePipelineButtons() {
  const job = state.lastJob;
  const shots = job?.shots || [];
  const activeShots = shots.filter((shot) => shot.generation_skipped !== true);
  const running = state.busy || job?.status === "running" || job?.status === "queued";
  const mosaicsReady = activeShots.length > 0 && activeShots.every((shot) => shot.has_mosaic);
  const whiteModelsReady = activeShots.length > 0 && activeShots.every(whiteModelQualityReady);
  const missingMosaicShots = activeShots.filter((shot) => !shot.has_mosaic).map((shot) => Number(shot.index));
  const privacyReview = $("#privacyReviewConfirmed");
  if (privacyReview && !mosaicsReady) privacyReview.checked = false;
  if (privacyReview) privacyReview.disabled = running || !mosaicsReady;
  if ($("#mosaicBtn")) $("#mosaicBtn").disabled = !shots.length || running;
  const needsCastReview = state.lastJob?.cast_continuity?.manual_review_required === true;
  if ($("#performanceBtn")) {
    $("#performanceBtn").disabled = !shots.length || running || (!needsCastReview && !$("#performanceConsent").checked);
    $("#performanceBtn").dataset.localReview = String(needsCastReview);
    $("#performanceBtn").textContent = needsCastReview ? "核对人物对应" : "分析全部分镜的台词与表演";
  }
  const mosaicHint = $("#mosaicStatusHint");
  if (mosaicHint) {
    mosaicHint.classList.toggle("ready", mosaicsReady);
    mosaicHint.textContent = !shots.length
      ? "请先完成免费分镜分析。"
      : running
        ? "当前任务正在运行，完成后可继续。"
        : mosaicsReady
          ? "全部有效分镜的打码视频已生成，可按需重新生成。"
          : `尚缺分镜 ${missingMosaicShots.map((value) => String(value).padStart(2, "0")).join("、")} 的打码视频；请先点击上方“生成完整打码视频”。`;
  }
  const whiteButton = $("#whiteModelBtn");
  const whiteBlockReason = !shots.length
    ? "请先完成免费分镜分析。"
    : running
      ? "当前任务正在运行，完成后可继续。"
      : !mosaicsReady
        ? `请先生成分镜 ${missingMosaicShots.map((value) => String(value).padStart(2, "0")).join("、")} 的打码视频。`
        : !privacyReview?.checked
          ? "请逐镜预览打码视频并勾选“确认没有漏脸”。"
          : "";
  if (whiteButton) {
    whiteButton.disabled = Boolean(whiteBlockReason);
    whiteButton.title = whiteBlockReason;
  }
  const whiteHint = $("#whiteModelStatusHint");
  if (whiteHint) {
    whiteHint.classList.toggle("ready", !whiteBlockReason);
    whiteHint.textContent = whiteBlockReason || (whiteModelsReady ? "全部白模分镜已生成，可复用或重新生成。" : "前置检查已完成，可以提交白模生成任务。需按镜计费。");
  }
  const needsCharacterUpload = Array.from({ length: state.actorCount }, (_value, offset) => offset + 1)
    .some((index) => !actorUsesAsset(index, existingActor(index)));
  const realActorBlockReason = !shots.length
    ? "请先完成分镜分析。"
    : running
      ? "当前任务正在运行，完成后才能上传人物角色。"
      : !$("#realPersonAuthorized").checked
        ? "请先勾选虚拟人物素材授权确认。"
        : needsCharacterUpload && !state.characterLibrary?.upload_ready
          ? "上传新人物还需配置火山素材库 AK/SK；也可以改选角色库已有 Active Asset。"
        : "";
  const realActorButton = $("#prepareRealActorsBtn");
  if (realActorButton) {
    realActorButton.disabled = Boolean(realActorBlockReason);
    realActorButton.title = realActorBlockReason;
  }
  const realActorHint = $("#realActorStatusHint");
  if (realActorHint) {
    realActorHint.classList.toggle("ready", !realActorBlockReason);
    realActorHint.textContent = realActorBlockReason || "前置条件已完成，可以上传新人物或绑定角色库已有 Asset。";
  }
  document.querySelectorAll("[data-regenerate-white-shot]").forEach((button) => {
    const shot = shots.find((item) => Number(item.index) === Number(button.dataset.regenerateWhiteShot));
    button.disabled = !mosaicsReady || running || !shot?.has_white_model;
    button.title = !$("#privacyReviewConfirmed").checked
      ? "点击后会提示你先重新确认打码视频没有漏脸。"
      : "只重新生成当前分镜；提交付费任务前会再次确认费用。";
  });
}

async function prepareRealActors() {
  try {
    if (!state.jobId || !state.shotCount) throw new Error("请先完成分镜分析。");
    if (!$("#realPersonAuthorized").checked) throw new Error("请先确认虚拟人物素材授权。");
    const form = new FormData();
    form.append("source_job_id", state.jobId);
    form.append("actor_count", String(state.actorCount));
    form.append("real_person_authorized", "true");
    let newUploadCount = 0;
    for (let index = 1; index <= state.actorCount; index += 1) {
      const role = $(`#roleDescription${index}`).value.trim();
      const person = $(`#personImage${index}`).files[0];
      const clothing = $(`#clothingImage${index}`).files[0];
      const existing = existingActor(index);
      const usesAsset = actorUsesAsset(index, existing);
      const assetUri = actorAssetUri(index, existing);
      if (!role) throw new Error(`请填写人物 ${index} 的角色定位。`);
      if (usesAsset && !/^asset:\/\/asset-[A-Za-z0-9_-]{6,120}$/.test(assetUri)) throw new Error(`请选择或填写人物 ${index} 的有效火山 Asset ID。`);
      if (!usesAsset && !person && !(existing?.has_person && !existing?.uses_authorized_asset)) throw new Error(`请上传人物 ${index} 的虚拟人物图片。`);
      if (!usesAsset) {
        if (!state.characterLibrary?.upload_ready) throw new Error(`人物 ${index} 无法上传角色库：请先配置火山素材库 AK/SK。`);
        const groupId = $(`#actorArkGroup${index}`).value;
        const assetName = $(`#actorArkName${index}`).value.trim();
        if (!groupId) throw new Error(`请为人物 ${index} 选择火山角色组。`);
        if (!assetName) throw new Error(`请填写人物 ${index} 的角色素材名称。`);
        form.append(`ark_group_id_${index}`, groupId);
        form.append(`ark_asset_name_${index}`, assetName);
        newUploadCount += 1;
      }
      form.append(`role_description_${index}`, role);
      form.append(`person_source_mode_${index}`, usesAsset ? "ark_asset" : "local_library");
      if (usesAsset) form.append(`person_asset_${index}`, assetUri);
      else if (person) form.append(`person_image_${index}`, person);
      if (clothing) form.append(`clothing_image_${index}`, clothing);
    }
    const uploadNote = newUploadCount ? `其中 ${newUploadCount} 位新人物会上传火山角色库并等待审核；` : "";
    if (!(await confirmAction(`将处理 ${state.actorCount} 位人物。${uploadNote}已有角色直接绑定 Active Asset。\n\n不会生成眼部遮挡图或素描图；最终成片的人物身份只引用角色库 Asset。是否继续？`, { title: "确认处理人物素材" }))) return;
    setBusy(true);
    const job = await api(`${LONG_API}/prepare-actors`, { method: "POST", body: form });
    monitor(job.id, "real-actors");
  } catch (error) { setBusy(false); toast(error.message, true); }
}

async function uploadSingleActorToLibrary(index) {
  try {
    if (!state.jobId || !state.shotCount) throw new Error("请先完成分镜分析。");
    if (state.busy || ["queued", "running"].includes(state.lastJob?.status)) throw new Error("当前任务正在运行，请完成或暂停后再上传人物。");
    if (!$("#realPersonAuthorized").checked) throw new Error("请先确认虚拟人物素材授权。");
    const existing = existingActor(index);
    if (actorUsesAsset(index, existing)) throw new Error(`人物 ${index} 当前选择的是角色库已有角色；如需上传新人物，请先点击“上传新人物”。`);
    const role = $(`#roleDescription${index}`).value.trim();
    const person = $(`#personImage${index}`).files[0];
    const clothing = $(`#clothingImage${index}`).files[0];
    const groupId = $(`#actorArkGroup${index}`).value;
    const assetName = $(`#actorArkName${index}`).value.trim();
    if (!role) throw new Error(`请填写人物 ${index} 的角色定位。`);
    if (!person && !(existing?.has_person && !existing?.uses_authorized_asset)) throw new Error(`请先为人物 ${index} 选择虚拟人物图片。`);
    if (!state.characterLibrary?.upload_ready) throw new Error(`人物 ${index} 暂时无法上传：${state.characterLibrary?.message || "请配置火山素材库 AK/SK。"}`);
    if (!groupId) throw new Error(`请为人物 ${index} 选择火山角色组。`);
    if (!assetName) throw new Error(`请填写人物 ${index} 的角色素材名称。`);
    if (!(await confirmAction(`确定将“${assetName}”上传到火山角色库吗？\n\n本次只处理人物 ${index}，会等待素材审核为 Active 后自动绑定。`, { title: "上传火山角色库", confirmLabel: "确认上传" }))) return;
    const form = new FormData();
    form.append("source_job_id", state.jobId);
    form.append("real_person_authorized", "true");
    form.append("role_description", role);
    form.append("ark_group_id", groupId);
    form.append("ark_asset_name", assetName);
    if (person) form.append("person_image", person);
    if (clothing) form.append("clothing_image", clothing);
    state.pendingActorUploadIndex = index;
    setBusy(true);
    const job = await api(`${LONG_API}/actors/${index}/character-asset`, { method: "POST", body: form });
    monitor(job.id, "real-actor");
  } catch (error) {
    state.pendingActorUploadIndex = null;
    setBusy(false);
    toast(error.message.trim(), true);
  }
}

async function deleteActorCharacterAsset(index) {
  const bound = existingActor(index)?.asset_uri || existingActor(index)?.ark_library_asset_uri || "";
  if (!canManageCharacterAsset(bound)) return toast("公司共享角色请在画布或创作中心管理。", true);
  const existing = existingActor(index);
  const assetUri = existing?.asset_uri || existing?.ark_library_asset_uri || "";
  if (!/^asset:\/\/asset-[A-Za-z0-9_-]{6,120}$/.test(assetUri)) return toast(`人物 ${index} 尚未绑定可删除的火山角色素材。`, true);
  if (state.busy || ["queued", "running"].includes(state.lastJob?.status)) return toast("当前任务正在运行，请完成或暂停后再删除人物素材。", true);
  const assetName = existing?.ark_library_asset_name || existing?.role || `人物 ${index}`;
  if (!(await confirmAction(`确定从火山角色库永久删除“${assetName}”吗？\n${assetUri}\n\n该操作不可撤销，引用该素材的旧任务可能无法再次生成。`, { title: "删除火山角色素材", confirmLabel: "永久删除", danger: true }))) return;
  const affectedIndices = (state.lastJob?.actors || [])
    .filter((actor) => (actor.asset_uri || actor.ark_library_asset_uri || "") === assetUri)
    .map((actor) => Number(actor.id));
  try {
    setBusy(true);
    const job = await api(`${LONG_API}/actors/${index}/character-asset`, {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ source_job_id: state.jobId, asset_uri: assetUri }),
    });
    render(job, false);
    for (const actorIndex of affectedIndices.length ? affectedIndices : [index]) {
      renderSingleActor(actorIndex, (job.actors || []).find((actor) => Number(actor.id) === actorIndex) || null);
      state.dirtyActorReferences.delete(actorIndex);
    }
    await loadCharacterLibrary({ rerenderActors: true });
    toast(`已从火山角色库删除：${assetName}`);
  } catch (error) {
    toast(error.message, true);
  } finally {
    setBusy(false);
  }
}

function setBusy(value) {
  state.busy = value;
  $("#analyzeBtn").disabled = value;
  if (!value) updateActorControls();
  updateCostAndButton();
  updatePipelineButtons();
  for (let index = 1; index <= state.actorCount; index += 1) syncActorLibraryUploadControl(index);
}

function appendActors(form) {
  const used = new Set([...state.shotCasts.values()].flat());
  form.append("actor_count", String(state.actorCount));
  for (let index = 1; index <= state.actorCount; index += 1) {
    const role = $(`#roleDescription${index}`).value.trim();
    const clothing = $(`#clothingImage${index}`).files[0];
    const existing = existingActor(index);
    const usesAsset = actorUsesAsset(index, existing);
    const assetUri = actorAssetUri(index, existing);
    if (!role) throw new Error(`请填写人物 ${index} 的原片角色定位。`);
    if (used.has(index) && (!usesAsset || state.dirtyActorReferences.has(index) || !/^asset:\/\/asset-[A-Za-z0-9_-]{6,120}$/.test(assetUri))) {
      throw new Error(`人物 ${index} 尚未绑定 Active 火山角色 Asset，请先点击“上传并绑定火山角色库”。`);
    }
    if (used.has(index) && !clothing && !existing?.has_clothing) throw new Error(`请提供人物 ${index} 的服装参考。`);
    form.append(`role_description_${index}`, role);
    form.append(`person_source_mode_${index}`, "ark_asset");
    if (usesAsset) form.append(`person_asset_${index}`, assetUri);
    if (clothing) form.append(`clothing_image_${index}`, clothing);
  }
}

function appendOptions(form) {
  const strategy = generationStrategy();
  const wholeGeneration = strategy === WHOLE_GENERATION_STRATEGY;
  form.append("generation_strategy", strategy);
  form.append("scene_prompt", $("#scenePrompt").value.trim());
  form.append("image_model", $("#imageModel").value.trim());
  form.append("blur_range", $("#blurRange").value.trim());
  form.append("prompt", $("#prompt").value.trim());
  form.append("final_style_id", $("#finalStyle").value || REAL_DEFAULT_FINAL_STYLE);
  form.append("model", $("#model").value.trim());
  form.append("resolution", $("#resolution").value);
  form.append("ratio", $("#ratio").value);
  form.append("generate_audio", "true");
  form.append("dialogue_voice_mode", "seedance_new_voice");
  form.append("watermark", "false");
  form.append("delete_tos_after", $("#deleteTos").checked ? "true" : "false");
  form.append("preserve_original_audio", "false");
  const sceneLibrary = state.sceneGroups.filter((group) => group.images.length).map((group) => ({
    id: group.id,
    name: group.name.trim() || "未命名场景",
    description: group.description.trim(),
    image_count: group.images.length,
  }));
  if (!sceneLibrary.length) throw new Error("请先在场景库上传新场景图片。");
  if (!wholeGeneration && !sceneMappingsComplete()) throw new Error("仍有分镜未匹配新场景，请点击自动匹配或逐镜选择。");
  if (wholeGeneration && !$("#wholeSceneReference").value) throw new Error("请选择整段成片使用的场景参考图。");
  form.append("whole_scene_reference", $("#wholeSceneReference").value || "");
  form.append("scene_library", JSON.stringify(sceneLibrary));
  for (const group of state.sceneGroups) {
    group.images.forEach((image, offset) => {
      if (image.file) form.append(`scene_group_${group.id}_${offset + 1}`, image.file);
    });
  }
  form.append("scene_assignments", JSON.stringify((state.lastJob?.shots || []).flatMap((shot) => {
    const assignment = sceneAssignmentFor(shot.index);
    if (!assignment) return [];
    return [{ index: Number(shot.index), group_id: assignment.groupId, image_index: Number(assignment.imageIndex) }];
  })));
}

async function analyze() {
  const videoFile = $("#referenceVideo").files[0];
  if (!videoFile) return toast("请先选择长视频。", true);
  if (window.DepthFlowWorkspaces) await window.DepthFlowWorkspaces.ready;
  const form = new FormData();
  form.append("reference_video", videoFile);
  form.append("sensitivity", $("#sensitivity").value);
  form.append("manual_cuts", $("#manualCuts").value.trim());
  window.DepthFlowWorkspaces?.appendToForm(form);
  state.shotCasts.clear(); state.manualCasts.clear(); state.shotScenes.clear(); state.manualSceneShots.clear(); invalidateCastConfirmation();
  state.preserveSceneLibraryOnNextJob = true;
  setBusy(true); $("#analysisState").textContent = "正在本地分析镜头与人数……";
  try {
    const job = await api(`${LONG_API}/analyze`, { method: "POST", body: form });
    state.jobId = job.id;
    await window.DepthFlowWorkspaces?.refresh();
    monitor(job.id, true);
  } catch (error) { setBusy(false); toast(error.message, true); }
}

async function prepareMosaic() {
  try {
    if (!state.jobId || !state.shotCount) throw new Error("请先完成分镜分析。");
    const form = new FormData();
    form.append("source_job_id", state.jobId);
    form.append("block_size", $("#blockSize").value);
    form.append("face_score_threshold", $("#faceScoreThreshold").value);
    setBusy(true);
    const job = await api(`${LONG_API}/mosaic`, { method: "POST", body: form });
    monitor(job.id, "mosaic");
  } catch (error) { setBusy(false); toast(error.message, true); }
}

function openCastReview() {
  showWorkflowStep("progress");
  $("#shotGrid").scrollIntoView({ behavior: "smooth", block: "start" });
  $("#shotGrid [data-person-mapping]")?.focus({ preventScroll: true });
}

async function analyzePerformance() {
  if (state.lastJob?.cast_continuity?.manual_review_required === true) {
    openCastReview();
    return;
  }
  try {
    if (!state.jobId || !state.shotCount) throw new Error("请先完成分镜分析。");
    if (!$("#performanceConsent").checked) throw new Error("请先确认临时上传原片分镜用于分析。");
    if (!(await confirmAction(`将调用 ${state.shotCount} 次逐镜多模态分析，提取原台词、说话人、语气和表演神色；并追加 1 次全片跨镜人物身份校验，避免人物换边后映射错位。\n\n这是云端 API 步骤，是否继续？`, { title: "确认开始台词与表演分析" }))) return;
    const form = new FormData();
    form.append("source_job_id", state.jobId);
    form.append("performance_consent", "true");
    setBusy(true);
    const job = await api(`${LONG_API}/performance`, { method: "POST", body: form });
    monitor(job.id, "performance");
  } catch (error) { setBusy(false); toast(error.message, true); }
}

function appendWhiteModelVideoOptions(form) {
  form.append("colored_cast", document.querySelector("#coloredCastEnabled")?.checked ? "true" : "false");
  const seedance20 = (state.config?.real_final_video_models || []).find((item) => item.id === "seedance_2_0");
  form.append("model", seedance20?.model || "doubao-seedance-2-0-260128");
  form.append("resolution", "480p");
  form.append("ratio", $("#whiteModelRatio").value);
  form.append("generate_audio", "false");
  form.append("watermark", "false");
  form.append("delete_tos_after", $("#deleteTos").checked ? "true" : "false");
}

function whiteModelForm(regenerationMode = "normal", forceShots = [], retryCostConfirmed = false) {
  if (!state.jobId || !state.shotCount) throw new Error("请先完成分镜分析。");
  const activeShots = (state.lastJob?.shots || []).filter((shot) => shot.generation_skipped !== true);
  if (!activeShots.length || !activeShots.every((shot) => shot.has_mosaic)) throw new Error("请先生成全部有效分镜的打码视频。");
  if (!$("#privacyReviewConfirmed").checked) throw new Error("请逐镜检查并确认没有漏脸。");
  const form = new FormData();
  form.append("source_job_id", state.jobId);
  form.append("privacy_review_confirmed", "true");
  form.append("white_regeneration_mode", regenerationMode);
  form.append("force_white_shots", JSON.stringify(forceShots));
  form.append("white_retry_cost_confirmed", retryCostConfirmed ? "true" : "false");
  appendWhiteModelVideoOptions(form);
  return form;
}

async function submitWhiteModel(regenerationMode, forceShots, confirmation, monitorMode = "white", retryCostConfirmed = false) {
  try {
    const form = whiteModelForm(regenerationMode, forceShots, retryCostConfirmed);
    const selectedRatio = $("#whiteModelRatio").value;
    const ratioNotice = selectedRatio === "adaptive"
      ? "\n\n白模画面尺寸：跟随原片（推荐），系统会逐镜识别原始比例。"
      : `\n\n白模画面尺寸：${selectedRatio}。若与原片不同，可能改变人物比例、景别和构图，请确认这是你的有意选择。`;
    if (!(await confirmAction(`${confirmation}${ratioNotice}`, { title: "确认生成白膜视频", confirmLabel: "确认并开始生成" }))) return;
    setBusy(true);
    const job = await api(`${LONG_API}/white-model`, { method: "POST", body: form });
    monitor(job.id, monitorMode);
  } catch (error) { setBusy(false); toast(error.message, true); }
}

async function generateWhiteModel() {
  return submitWhiteModel(
    "normal",
    [],
    `将按 ${state.shotCount} 个分镜检查并提交需要生成的 Seedance 白模任务。\n\n白模固定使用 Seedance 2.0 / 480p 低成本规格；最终重绘仍使用你选择的成片分辨率。系统只使用打码视频作为参考，配置未变化且通过质量闸门的已有白模会复用。\n\n每个新白模完成后会本地检查人物占位、动作/运镜、绿幕、白色材质和疑似文字；未通过时最多自动付费纠偏 1 次，仍失败则停止并等待你处理，不会进入成片。\n\n这是付费步骤，是否继续？`,
  );
}

async function regenerateWhiteModelShot(index) {
  const shot = (state.lastJob?.shots || []).find((item) => Number(item.index) === Number(index));
  if (!shot?.has_mosaic) return toast(`分镜 ${String(index).padStart(2, "0")} 还没有可用的打码视频，不能生成白模。`, true);
  const completedAttempts = Number(shot.white_model_total_generation_count || 0)
    || (shot.has_white_model ? Math.max(1, Number(shot.white_model_retry_count || 0) + 1) : 0);
  const needsRenewedCostConfirmation = completedAttempts >= 2;
  const confirmation = needsRenewedCostConfirmation
    ? `分镜 ${String(index).padStart(2, "0")} 已完成 ${completedAttempts} 次付费白模生成，自动纠偏额度已经用完。\n\n本次经你确认后，只会额外提交 1 个 Seedance 2.0 / 480p 付费任务；无论质检是否通过，都不会自动追加下一次付费生成。其他分镜不会改动。\n\n确认承担本次单镜生成费用并继续吗？`
    : `只重新生成分镜 ${String(index).padStart(2, "0")} 的白模视频。\n\n会按 Seedance 2.0 / 480p 新建 1 次付费任务；若本地质量闸门未通过，最多自动付费纠偏 1 次。其他已通过白模直接复用，完成后重新检查是否可以合并。最终重绘成片不会自动改动。\n\n是否继续？`;
  return submitWhiteModel(
    "selected",
    [Number(index)],
    confirmation,
    "white-shot",
    needsRenewedCostConfirmation,
  );
}

function castPayload() {
  return (state.lastJob?.shots || []).map((shot) => {
    const actorIds = castFor(shot.index);
    return {
      index: Number(shot.index),
      actor_ids: actorIds,
      actor_mappings: actorIds.map((actorId, offset) => ({
        slot: offset + 1,
        actor_id: actorId,
        position_lock: positionLockValue(shot, offset + 1),
      })),
      manual_identity_override: state.manualCasts.has(Number(shot.index)) || shot.manual_identity_override === true,
    };
  });
}

function generationForm(regenerationMode = "normal", forceShots = [], compositionRetryApproved = false, skipShotIndex = 0) {
  if (!state.jobId || !state.shotCount) throw new Error("请先完成分镜分析。");
  if (!state.castConfirmed) throw new Error("请先核对全部分镜角色并勾选确认。");
  if (generationStrategy() !== WHOLE_GENERATION_STRATEGY && !sceneMappingsComplete()) throw new Error("仍有分镜没有匹配新场景。");
  const form = new FormData();
  form.append("source_job_id", state.jobId);
  form.append("shot_casts", JSON.stringify(castPayload()));
  form.append("cast_confirmed", "true");
  form.append("require_white_model", $("#requireWhiteModel").checked ? "true" : "false");
  form.append("regeneration_mode", regenerationMode);
  form.append("force_shots", JSON.stringify(forceShots));
  form.append("composition_retry_approved", compositionRetryApproved ? "true" : "false");
  form.append("skip_shot_index", String(Number(skipShotIndex) || 0));
  appendActors(form); appendOptions(form);
  return form;
}

async function submitGeneration(regenerationMode, forceShots, confirmation, compositionRetryApproved = false, skipShotIndex = 0) {
  try {
    const form = generationForm(regenerationMode, forceShots, compositionRetryApproved, skipShotIndex);
    if (!(await confirmAction(confirmation, { title: "确认生成成片", confirmLabel: "确认并开始生成" }))) return;
    setBusy(true);
    const job = await api(`${LONG_API}/generate`, { method: "POST", body: form });
    monitor(job.id, "final");
  } catch (error) {
    setBusy(false);
    try {
      if (state.jobId) render(await api(`/api/jobs/${state.jobId}`), false);
    } catch (_refreshError) {
      // Keep the original submission error visible if state refresh is unavailable.
    }
    toast(error.message, true);
  }
}

async function approveCompositionRetry(index) {
  const shot = (state.lastJob?.shots || []).find((item) => Number(item.index) === Number(index));
  if (!shot?.composition_approval_required) return toast("本镜头当前不需要人工同意。", true);
  const retryCount = Number(shot.composition_retry_count || 0);
  const unfinishedCount = (state.lastJob?.shots || []).filter(
    (item) => Number(item.index) !== Number(index) && !item.has_output,
  ).length;
  const continuationNote = unfinishedCount
    ? `如果本次纠偏通过，系统会按顺序继续生成其余 ${unfinishedCount} 个尚未完成的分镜；任一分镜再次达到纠偏上限时，会在提交下一镜前停止。`
    : "如果本次纠偏通过，系统会复用其他已完成分镜并重新合并整片。";
  return submitGeneration(
    "selected",
    [Number(index)],
    `分镜 ${String(index).padStart(2, "0")} 已完成 ${retryCount} 次构图纠偏，仍未通过自动验收。\n\n继续会先为本镜创建 1 次 Seedance 付费纠偏任务。${continuationNote}\n\n是否同意继续？`,
    true,
  );
}

async function approveWhiteModel(index) {
  const shot = (state.lastJob?.shots || []).find((item) => Number(item.index) === Number(index));
  if (!shot?.has_white_model) return toast("本镜头没有可确认的白模。", true);
  const reasons = (shot?.white_model_qa?.reasons || []).join("；") || "新版质量检查未通过";
  if (!(await confirmAction(`分镜 ${String(index).padStart(2, "0")} 的白模未通过自动质量闸门：\n${reasons}\n\n只有你已逐帧确认人物数量、站位、动作、绿幕、白色素体以及无文字均可接受时，才应继续。人工确认不会产生费用，但之后生成成片会产生Seedance费用。`, { title: `确认使用分镜 ${String(index).padStart(2, "0")} 白模`, confirmLabel: "确认使用此白模" }))) return;
  try {
    setBusy(true);
    const job = await api(`${LONG_API}/${state.jobId}/shots/${Number(index)}/approve-white-model`, { method: "POST" });
    render(job);
    toast(`分镜 ${String(index).padStart(2, "0")} 白模已人工确认；最终成片仍会执行二次验收。`);
  } catch (error) {
    setBusy(false);
    toast(error.message, true);
  }
}

async function skipShotAndContinue(index) {
  const shot = (state.lastJob?.shots || []).find((item) => Number(item.index) === Number(index));
  if (!shot || shot.generation_skipped === true) return toast("本镜头已经处于跳过状态。", true);
  const remaining = (state.lastJob?.shots || []).filter(
    (item) => Number(item.index) > Number(index)
      && item.generation_skipped !== true
      && !item.has_output,
  ).length;
  const continuation = remaining
    ? `系统会从后续尚未完成的 ${remaining} 个分镜继续生成。`
    : "系统会直接使用其他未跳过的已有片段重新合并整片。";
  return submitGeneration(
    "normal",
    [],
    `确定跳过分镜 ${String(index).padStart(2, "0")} 吗？\n\n本镜不会再次提交 Seedream 或 Seedance，也不会进入最终整片；当前偏差结果仍保留供预览和下载。${continuation}\n\n以后可点击“恢复此镜”重新加入流程。`,
    false,
    Number(index),
  );
}

async function restoreSkippedShot(index) {
  const shot = (state.lastJob?.shots || []).find((item) => Number(item.index) === Number(index));
  if (!shot?.generation_skipped) return toast("本镜头当前没有被跳过。", true);
  if (!(await confirmAction(`恢复并单独重新生成分镜 ${String(index).padStart(2, "0")} 吗？\n\n恢复状态本身不收费，随后会为本镜新建 1 次 Seedance 付费任务；系统不会复用跳过前的偏差成片。其他暂停、失败或待纠偏镜头不会阻止本镜，也不会被混入整片。`, { title: "恢复并重新生成分镜", confirmLabel: "确认付费并重生" }))) return;
  try {
    setBusy(true);
    const restored = await api(`${LONG_API}/${state.jobId}/shots/${Number(index)}/restore`, { method: "POST" });
    render(restored);
    const form = generationForm("selected", [Number(index)], false);
    const job = await api(`${LONG_API}/generate`, { method: "POST", body: form });
    monitor(job.id, "final");
  } catch (error) {
    setBusy(false);
    try {
      if (state.jobId) render(await api(`/api/jobs/${state.jobId}`), false);
    } catch (_refreshError) {
      // Preserve the original restore/generation error.
    }
    toast(error.message, true);
  }
}

async function generate() {
  const prerequisiteMessage = String($("#generateBtn")?.dataset.blockers || "").trim();
  if (prerequisiteMessage) {
    return toast(`暂不能生成：${prerequisiteMessage}。`, true);
  }
  const wholeGeneration = generationStrategy() === WHOLE_GENERATION_STRATEGY;
  const pendingCompositionShots = (state.lastJob?.shots || [])
    .filter((shot) => shot.composition_approval_required && shot.generation_skipped !== true)
    .map((shot) => Number(shot.index));
  if (!wholeGeneration && pendingCompositionShots.length === 1) {
    return approveCompositionRetry(pendingCompositionShots[0]);
  }
  if (!wholeGeneration && pendingCompositionShots.length > 1) {
    const firstIndex = pendingCompositionShots[0];
    document.querySelector(`[data-shot-index="${firstIndex}"]`)?.scrollIntoView({ behavior: "smooth", block: "center" });
    return toast(
      `分镜 ${pendingCompositionShots.map((value) => String(value).padStart(2, "0")).join("、")} 均在等待人工处理；请逐镜选择继续纠偏、重新生成或跳过。`,
      true,
    );
  }
  const selectedShots = castPayload();
  const empty = selectedShots.filter((item) => !item.actor_ids.length).length;
  if (wholeGeneration) {
    const sceneLabel = $("#wholeSceneReference").selectedOptions[0]?.textContent || "所选场景图";
    return submitGeneration(
      "normal",
      [],
      `将使用模式二生成完整成片：\n\n• 把已合并白模作为整段唯一动作、切点、景别、构图和运镜母版\n• 固定使用 Seedance 2.5，只创建 1 次付费视频任务\n• 直接参考“${sceneLabel}”，不再生成任何逐镜 Seedream 场景板\n• 固定场景提示：@场景图 只做场景参考，不做必要背景。\n• 原片音轨不会提交；新台词与音色由 Seedance 按人工校订时间轴生成\n\n整段模式不能单镜重生，生成后将直接得到完整成片。是否提交？`,
    );
  }
  return submitGeneration(
    "normal",
    [],
    `已核对 ${selectedShots.length} 个分镜\n\n系统会复用配置未变化的已有成片；其余分镜将重新生成场景板与 Seedance 视频。\n其中无人场景重绘 ${empty} 个，不再保留原画面。\n\n系统会先在本地隔离原片对白，只让 Seedance 参考逐字语速、停顿、重音与情绪并生成新人物音色；原片对白、BGM和环境音均不会合回，最终音轨完全来自Seedance直生。\n\n这是付费步骤，是否提交？`,
  );
}

async function regenerateAll() {
  const shotIndexes = (state.lastJob?.shots || [])
    .filter((shot) => shot.generation_skipped !== true)
    .map((shot) => Number(shot.index));
  return submitGeneration(
    "all",
    shotIndexes,
    `将强制重新生成全部 ${shotIndexes.length} 个分镜。\n\n会新建 ${shotIndexes.length} 次 Seedance 付费任务；白模与未变化的场景板会复用，场景配置变化时会补建 Seedream 场景板。\n每镜提交的动作参考完全静音；Seedance 只按人工校订后的说话人、台词和时间轴生成新人物声音，原片对白、BGM、歌声与环境音都不会进入参考或成片。\n完成后自动重新合并整片。\n\n是否继续？`,
  );
}

async function regenerateShot(index) {
  const shot = (state.lastJob?.shots || []).find((item) => Number(item.index) === Number(index));
  if (!shot?.has_output) return toast(`分镜 ${String(index).padStart(2, "0")} 还没有可重新生成的成片。`, true);
  return submitGeneration(
    "selected",
    [Number(index)],
    `只重新生成分镜 ${String(index).padStart(2, "0")}。\n\n会新建 1 次 Seedance 付费任务；白模与未变化的场景板会复用。其他暂停、失败、待纠偏或旧配置分镜不会阻止本镜，也不会被自动重生或混入整片。\n本镜提交的动作参考完全静音；Seedance 只按人工校订后的说话人、台词和时间轴生成新人物声音，原片对白、BGM、歌声与环境音都不会进入参考或成片。\n如果其他未跳过分镜均已完成且配置兼容，系统会自动重新合并；否则只保存本镜结果，整片等待后续补齐。\n\n是否继续？`,
  );
}

async function savePerformanceEdits(index) {
  const card = $(`[data-shot-index="${index}"]`);
  const dialogue = card?.querySelector(`[data-performance-dialogue="${index}"]`)?.value ?? "";
  const performance = card?.querySelector(`[data-performance-acting="${index}"]`)?.value ?? "";
  const dialogueSources = Array.from(card?.querySelectorAll("[data-dialogue-source-index]") || []).map((node) => {
    const value = String(node.value || "uncertain");
    return {
      index: Number(node.dataset.dialogueSourceIndex),
      source_type: value.startsWith("p") ? "character" : value,
      speaker_slot: value.startsWith("p") ? Number(value.slice(1)) : 0,
    };
  });
  const button = card?.querySelector(`[data-save-performance="${index}"]`);
  if (!state.jobId) return toast("请先完成分镜分析。", true);
  if (dialogue.length + performance.length > 2600) return toast("本分镜台词与表演合计最多 2600 字，请精简后保存。", true);
  if (button) { button.disabled = true; button.textContent = "正在保存…"; }
  try {
    const job = await api(`${LONG_API}/${state.jobId}/shots/${index}/performance`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ dialogue_text: dialogue, performance_text: performance, dialogue_sources: dialogueSources }),
    });
    state.performanceDrafts.delete(Number(index));
    render(job);
    toast(`分镜 ${String(index).padStart(2, "0")} 的台词与表演已保存；生成时将优先使用人工文字。`);
  } catch (error) {
    if (button) { button.disabled = false; button.textContent = "保存本镜校订"; }
    toast(error.message, true);
  }
}

function resetPerformanceEditor(index) {
  const shot = (state.lastJob?.shots || []).find((item) => Number(item.index) === Number(index));
  const card = $(`[data-shot-index="${index}"]`);
  if (!shot || !card) return;
  const dialogue = extractedDialogueText(shot.performance || {});
  const performance = extractedPerformanceText(shot.performance || {});
  card.querySelector(`[data-performance-dialogue="${index}"]`).value = dialogue;
  card.querySelector(`[data-performance-acting="${index}"]`).value = performance;
  state.performanceDrafts.set(Number(index), { dialogue, performance });
  toast(`分镜 ${String(index).padStart(2, "0")} 已恢复为 AI 原始提取文字，点击保存后才会生效。`);
}

async function deleteShot(index) {
  const shots = state.lastJob?.shots || [];
  const shot = shots.find((item) => Number(item.index) === Number(index));
  if (!shot) return toast(`找不到分镜 ${String(index).padStart(2, "0")}。`, true);
  if (shots.length <= 1) return toast("至少需要保留一个分镜。", true);
  const duration = Number(shot.duration || 0).toFixed(2);
  if (!(await confirmAction(`删除分镜 ${String(index).padStart(2, "0")}（${duration} 秒）？\n\n该镜将从后续白模生成和最终合并中排除；本地原分镜文件仍会保留。`, { title: "删除分镜", confirmLabel: "确认删除", danger: true }))) return;
  try {
    setBusy(true);
    const job = await api(`${LONG_API}/${state.lastJob.id}/shots/${Number(index)}`, { method: "DELETE" });
    state.shotCasts.delete(Number(index));
    state.manualCasts.delete(Number(index));
    state.shotScenes.delete(Number(index));
    state.manualSceneShots.delete(Number(index));
    state.castConfirmed = false;
    render(job, false);
    toast(`分镜 ${String(index).padStart(2, "0")} 已删除，剩余 ${job.shot_count} 个分镜。`);
  } catch (error) {
    toast(error.message, true);
  } finally {
    setBusy(false);
  }
}

function video(selector, url, key) {
  const node = $(selector);
  if (node.dataset.key === key) return;
  node.src = `${url}?preview=${encodeURIComponent(key)}`;
  node.dataset.key = key;
  node.load();
}

function downloadCenterLink(url, name, label, primary = false) {
  if (!url) return "";
  return `<a class="${primary ? "primary-button" : "secondary-button"}" href="${escapeHtml(url)}?download=1" data-result-download-url="${escapeHtml(url)}" data-result-download-name="${escapeHtml(name)}">${escapeHtml(label)}</a>`;
}

function downloadShotRows(job, artifact) {
  const isWhite = artifact === "white";
  return (job.shots || []).map((shot) => {
    const index = Number(shot.index);
    const available = isWhite ? shot.has_white_model : shot.has_raw_output;
    const url = isWhite ? shot.white_model_url : shot.raw_output_url;
    const fileName = `${isWhite ? "白模" : "裁剪前成片"}_分镜${String(index).padStart(2, "0")}_${job.id}.mp4`;
    const skipped = shot.generation_skipped === true ? " · 已跳过，不进入整片" : "";
    return `<div class="download-shot-row"><span><b>分镜 ${String(index).padStart(2, "0")}</b><small>${Number(shot.start || 0).toFixed(2)}–${Number(shot.end || 0).toFixed(2)}秒 · ${Number(shot.duration || 0).toFixed(2)}秒${skipped}</small></span>${available ? downloadCenterLink(url, fileName, isWhite ? "下载白模片段" : "下载裁剪前成片") : `<em>${isWhite ? "白模" : "裁剪前成片"}尚未生成</em>`}</div>`;
  }).join("");
}

function renderDownloadCenter(job) {
  const center = $("#downloadCenter");
  const hasWhite = Boolean(job.has_white_model || (job.shots || []).some((shot) => shot.has_white_model));
  const hasFinal = Boolean(job.has_output || (job.shots || []).some((shot) => shot.has_raw_output));
  center.classList.toggle("hidden", !hasWhite && !hasFinal);
  if (!hasWhite && !hasFinal) return;
  const bundleLinks = [];
  if (job.has_white_model) bundleLinks.push(downloadCenterLink(job.white_model_url, `完整白模绿幕视频_${job.id}.mp4`, "下载完整白模视频"));
  if (job.has_output) bundleLinks.push(downloadCenterLink(job.output_url, `完整最终成片_${job.id}.mp4`, "下载整合最终成片", true));
  $("#downloadBundleLinks").innerHTML = bundleLinks.join("");
  $("#whiteShotDownloadList").innerHTML = downloadShotRows(job, "white");
  $("#finalShotDownloadList").innerHTML = downloadShotRows(job, "final");
}

function render(job, analysisOnly = false) {
  // A completed job must never inherit a stale local submission lock. This can
  // happen when the service finishes or restarts while the browser tab stays open.
  if (!["queued", "running"].includes(job.status)) state.busy = false;
  const firstRender = state.lastJob?.id !== job.id;
  if (firstRender) {
    // Per-shot choices belong to one analysis job. Keeping them while switching jobs
    // can force a newly detected multi-person shot back to the previous one-person map.
    state.shotCasts.clear();
    state.manualCasts.clear();
    state.shotScenes.clear();
    state.manualSceneShots.clear();
    state.castConfirmed = false;
    state.performanceDrafts.clear();
    state.positionDrafts.clear();
    initializeSceneLibrary(job, state.preserveSceneLibraryOnNextJob);
    state.preserveSceneLibraryOnNextJob = false;
  }
  state.lastJob = job; state.jobId = job.id; state.activeJobId = job.id; state.shotCount = job.shot_count || 0;
  if (job.generation_strategy === WHOLE_GENERATION_STRATEGY && generationStrategy() !== WHOLE_GENERATION_STRATEGY) {
    $("#generationStrategyWhole").checked = true;
    syncGenerationStrategy({ keepResolution: true });
  }
  const detectedMax = Math.max(0, ...(job.shots || []).map(detectedPeopleCount));
  const requiredActorCount = Math.max(job.actors?.length || 0, detectedMax);
  if (firstRender && requiredActorCount) {
    state.actorCount = Math.max(1, Math.min(2, requiredActorCount));
    renderActorList(job.actors);
  }
  renderActorReferencePreviews(job.actors || []);
  applyAutomaticCastSuggestions();
  if (!state.manualCasts.size && sceneMappingsComplete() && (job.shots || []).length && (job.shots || []).every((shot) => shot.cast_confirmed === true)) {
    state.castConfirmed = true;
  }
  $("#emptyTask").classList.add("hidden"); $("#activeTask").classList.remove("hidden");
  const kindLabels = {
    long_generate: "逐分镜最终重绘与合并",
    long_mosaic: "本地多脸时序打码",
    long_performance: "台词与表演神色分析",
    long_white_model: "白模绿幕母版生成",
    long_edit: "分镜编辑",
  };
  $("#jobKind").textContent = job.kind === "long_generate" && job.generation_strategy === WHOLE_GENERATION_STRATEGY
    ? "整段白模 · Seedance 2.5 一次生成"
    : kindLabels[job.kind] || "本地分镜与人数分析";
  $("#jobStage").textContent = job.stage;
  $("#jobMeta").textContent = `本地任务：${job.id}${job.source_duration ? ` · 原片 ${Number(job.source_duration).toFixed(2)} 秒` : ""}`;
  $("#progressPercent").textContent = `${job.progress}%`; $("#progressBar").style.width = `${job.progress}%`;
  $("#jobLogs").textContent = job.logs.join("\n"); $("#jobLogs").scrollTop = $("#jobLogs").scrollHeight;
  renderShotGrid();
  $("#castConfirmBar").classList.toggle("hidden", !state.shotCount);
  $("#confirmCast").checked = state.castConfirmed;
  $("#continuityReviewNotice").classList.toggle("hidden", job.cast_continuity?.manual_review_required !== true);
  $("#continuityReviewDetail").textContent = "台词、表演和打码结果已保留。自动分析未能确定全部人物对应，请逐镜选择人物；只核对本地结果，不会再次上传或收费。";

  if (state.shotCount) {
    const manualContinuity = job.cast_continuity?.manual_review_required === true;
    $("#analysisState").textContent = manualContinuity
      ? `已拆分 ${state.shotCount} 个分镜；跨镜自动身份不可用，请逐镜人工确认每个 P 槽位对应的新人物`
      : `已拆分 ${state.shotCount} 个分镜，请核对每镜角色`;
  }
  updateActorControls(); updateCostAndButton();
  $("#mosaicPreviewArea").classList.toggle("hidden", !job.has_mosaic);
  if (job.has_mosaic) {
    video("#mosaicVideo", job.mosaic_url, `${job.id}-mosaic-${job.created_at}`);
    $("#mosaicDownloadLink").href = `${job.mosaic_url}?download=1`;
    $("#mosaicDownloadLink").dataset.downloadUrl = job.mosaic_url;
    $("#mosaicDownloadLink").dataset.suggestedName = `人脸打码视频_${job.id}.mp4`;
  }
  $("#whitePreviewArea").classList.toggle("hidden", !job.has_white_model);
  if (job.has_white_model) {
    const whiteRevision = (job.shots || []).map((shot) => shot.white_model_task_id || shot.white_model_signature || "none").join("-");
    video("#whiteModelVideo", job.white_model_url, `${job.id}-white-${job.created_at}-${whiteRevision}`);
    $("#whiteModelDownloadLink").href = `${job.white_model_url}?download=1`;
    $("#whiteModelDownloadLink").dataset.downloadUrl = job.white_model_url;
    $("#whiteModelDownloadLink").dataset.suggestedName = `白模绿幕视频_${job.id}.mp4`;
  }
  $("#finalResultArea").classList.toggle("hidden", !job.has_output);
  if (job.has_output) {
    const wholeOutput = job.generation_strategy === WHOLE_GENERATION_STRATEGY;
    const outputRevision = wholeOutput
      ? `${job.task_id || job.created_at}-whole`
      : (job.shots || []).map((shot) => shot.task_id || shot.output_signature || "none").join("-");
    video("#finalResultVideo", job.output_url, `${job.id}-final-${job.created_at}-${outputRevision}`);
    $("#finalResultArea .preview-heading b").textContent = wholeOutput ? "整段白模一次生成成片" : "多分镜合并成片";
    $("#finalResultArea .preview-heading small").textContent = wholeOutput
      ? "同一个 Seedance 2.5 任务保持全片场景连续"
      : "已按原始顺序与时长合并";
    $("#finalDownloadLink").href = `${job.output_url}?download=1`;
    $("#finalDownloadLink").dataset.downloadUrl = job.output_url;
    $("#finalDownloadLink").dataset.suggestedName = wholeOutput
      ? `整段白模一次生成_${job.id}.mp4`
      : `长视频多分镜复刻_${job.id}.mp4`;
  }
  renderDownloadCenter(job);
  if (analysisOnly && job.status === "succeeded") $("#shotGrid").scrollIntoView({ behavior: "smooth", block: "start" });
}

function monitorModeForJob(job) {
  const modes = {
    long_mosaic: "mosaic",
    long_performance: "performance",
    long_white_model: "white",
    long_generate: "final",
    real_long_actor_prepare: "real-actors",
  };
  return modes[job?.kind] || true;
}

function monitor(id, mode, { scroll = true } = {}) {
  clearInterval(state.pollTimer);
  const poll = async () => {
    try {
      const job = await api(`/api/jobs/${id}`);
      render(job, mode === true || mode === "analysis");
      if (["succeeded", "failed", "awaiting_approval", "paused"].includes(job.status)) {
        clearInterval(state.pollTimer); setBusy(false);
        const successMessages = {
          mosaic: "打码视频已完成，请逐镜检查是否有漏脸。",
          performance: "全部分镜的台词与表演信息已提取。",
          white: "白模绿幕视频已按原时长合并完成。",
          "white-shot": "所选分镜白模已重新生成，并重新合并完整白模视频。",
          final: "最终重绘视频已生成并合并完成。",
          "real-actors": "人物已上传并绑定火山角色库，可继续配置服装与生成成片。",
          "real-actor": "所选人物已上传并绑定火山角色库。",
        };
        if (job.status === "succeeded") {
          if (mode === "real-actors" || mode === "real-actor") {
            state.actorCount = Math.max(1, Math.min(2, job.actors?.length || state.actorCount));
            if (mode === "real-actor" && state.pendingActorUploadIndex) {
              const uploadedIndex = state.pendingActorUploadIndex;
              state.dirtyActorReferences.delete(uploadedIndex);
              renderSingleActor(uploadedIndex, (job.actors || []).find((actor) => Number(actor.id) === uploadedIndex) || null);
            } else {
              state.dirtyActorReferences.clear();
              renderActorList(job.actors || []);
            }
            state.pendingActorUploadIndex = null;
            // Re-evaluate after the newly Active Ark asset has replaced the local
            // upload as the only allowed final identity source.
            updateCostAndButton();
          }
          const manualContinuity = job.cast_continuity?.manual_review_required === true;
          if (manualContinuity && mode === "performance") {
            toast("台词与表演已保存，请点击“核对人物对应”继续。");
            $("#continuityReviewNotice").scrollIntoView({ behavior: "smooth", block: "center" });
          } else toast((mode === true || mode === "analysis")
            ? manualContinuity
              ? `逐镜台词与表演已完成；跨镜自动身份失败，请逐镜人工选择人物后继续。`
              : `分镜与人数分析完成：${job.shot_count}个，请逐镜核对角色。`
            : (successMessages[mode] || "任务已完成。"), manualContinuity);
        }
        else if (job.status === "awaiting_approval") toast("自动构图纠偏已达到上限，请预览对应分镜后决定是否继续付费生成。", true);
        else if (job.status === "paused") toast("真实人物生成已暂停；不会提交后续分镜。当前云端任务ID已保留。");
        else {
          if (mode === "real-actor") state.pendingActorUploadIndex = null;
          toast(window.DepthFlowUI?.formatJobError?.(job) || job.error || "任务失败。", true);
        }
      }
    } catch (error) { clearInterval(state.pollTimer); setBusy(false); toast(error.message, true); }
  };
  poll(); state.pollTimer = setInterval(poll, 1800);
  if (scroll) $(".task-panel").scrollIntoView({ behavior: "smooth", block: "start" });
}

async function openFolder() {
  if (!state.activeJobId) return;
  try { await api(`/api/jobs/${state.activeJobId}/open-folder`, { method: "POST" }); } catch (error) { toast(error.message, true); }
}

async function pauseGeneration() {
  if (!state.jobId || !["queued", "running"].includes(state.lastJob?.status)) return;
  try {
    const job = await api(`${LONG_API}/pause`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ source_job_id: state.jobId }),
    });
    render(job);
    toast("暂停请求已发送：当前云端任务可能继续完成，但不会自动纠偏或提交下一分镜。");
  } catch (error) { toast(error.message, true); }
}

async function saveFile(url, name, type) {
  if (typeof window.showSaveFilePicker !== "function") { const anchor = document.createElement("a"); anchor.href = `${url}?download=1`; anchor.download = name; anchor.click(); return; }
  try {
    const picker = type === "image"
      ? { description: "JPEG 图片", accept: { "image/jpeg": [".jpg", ".jpeg"] } }
      : type === "json"
        ? { description: "JSON 数据", accept: { "application/json": [".json"] } }
        : { description: "MP4 视频", accept: { "video/mp4": [".mp4"] } };
    const handle = await window.showSaveFilePicker({ suggestedName: name, types: [picker] });
    const response = await fetch(url); const writable = await handle.createWritable();
    if (response.body) await response.body.pipeTo(writable); else { await writable.write(await response.blob()); await writable.close(); }
    toast(`已保存：${handle.name}`);
  } catch (error) { if (error?.name !== "AbortError") toast(error.message, true); }
}

$("#shotGrid").addEventListener("change", (event) => {
  const sceneGroupSelect = event.target.closest("[data-shot-scene-group]");
  if (sceneGroupSelect) {
    const shotIndex = Number(sceneGroupSelect.dataset.shotSceneGroup);
    const group = sceneGroupById(sceneGroupSelect.value);
    if (group?.images.length) state.shotScenes.set(shotIndex, { groupId: group.id, imageIndex: 1 });
    else state.shotScenes.delete(shotIndex);
    state.manualSceneShots.add(shotIndex);
    renderShotGrid(); invalidateCastConfirmation(); return;
  }
  const sceneImageSelect = event.target.closest("[data-shot-scene-image]");
  if (sceneImageSelect) {
    const shotIndex = Number(sceneImageSelect.dataset.shotSceneImage);
    const current = sceneAssignmentFor(shotIndex);
    if (current) state.shotScenes.set(shotIndex, { ...current, imageIndex: Number(sceneImageSelect.value) });
    state.manualSceneShots.add(shotIndex);
    renderShotGrid(); invalidateCastConfirmation(); return;
  }
  const select = event.target.closest("[data-person-mapping]");
  if (!select) return;
  const shotIndex = Number(select.dataset.personMapping);
  const slotIndex = Number(select.dataset.personSlot) - 1;
  const actorId = Number(select.value);
  const current = [...castFor(shotIndex)];
  const previousActor = current[slotIndex];
  const duplicateIndex = current.indexOf(actorId);
  if (duplicateIndex >= 0 && duplicateIndex !== slotIndex) current[duplicateIndex] = previousActor;
  current[slotIndex] = actorId;
  state.shotCasts.set(shotIndex, current); state.manualCasts.add(shotIndex);
  refreshShotCast(shotIndex); invalidateCastConfirmation();
  toast(`只修改了分镜 ${String(shotIndex).padStart(2, "0")} 的 P${slotIndex + 1}，其他分镜保持不变。`);
});

$("#shotGrid").addEventListener("input", (event) => {
  const positionLock = event.target.closest("[data-position-lock]");
  if (positionLock) {
    const key = `${Number(positionLock.dataset.positionLock)}:${Number(positionLock.dataset.positionSlot)}`;
    state.positionDrafts.set(key, positionLock.value);
    invalidateCastConfirmation();
    return;
  }
  const dialogue = event.target.closest("[data-performance-dialogue]");
  const acting = event.target.closest("[data-performance-acting]");
  if (!dialogue && !acting) return;
  const index = Number((dialogue || acting).dataset.performanceDialogue || (dialogue || acting).dataset.performanceActing);
  const draft = state.performanceDrafts.get(index) || {};
  if (dialogue) draft.dialogue = dialogue.value;
  if (acting) draft.performance = acting.value;
  state.performanceDrafts.set(index, draft);
});

$("#shotGrid").addEventListener("click", (event) => {
  const compareButton = event.target.closest("[data-shot-compare-src]");
  if (compareButton) {
    const comparison = compareButton.closest(".shot-comparison");
    const video = comparison?.querySelector(".shot-preview");
    if (!video) return;
    video.pause();
    video.removeAttribute("src");
    video.dataset.loadedSource = "";
    video.dataset.mediaSrc = compareButton.dataset.shotCompareSrc;
    comparison.querySelectorAll("[data-shot-compare-src]").forEach((button) => button.classList.toggle("active", button === compareButton));
    const caption = comparison.querySelector(".shot-preview-caption");
    if (caption) caption.textContent = `当前预览：${compareButton.dataset.shotCompareLabel}`;
    const loadButton = comparison.querySelector("[data-load-shot-preview]");
    if (loadButton) loadButton.textContent = `加载${compareButton.dataset.shotCompareLabel}预览`;
    activateShotPreview(video, () => {}, true);
    return;
  }
  const previewButton = event.target.closest("[data-load-shot-preview]");
  if (previewButton) {
    const card = previewButton.closest(".shot-card");
    activateShotPreview(card?.querySelector(".shot-preview"), () => {}, true);
    return;
  }
  const savePerformanceButton = event.target.closest("[data-save-performance]");
  if (savePerformanceButton) {
    savePerformanceEdits(Number(savePerformanceButton.dataset.savePerformance));
    return;
  }
  const resetPerformanceButton = event.target.closest("[data-reset-performance]");
  if (resetPerformanceButton) {
    resetPerformanceEditor(Number(resetPerformanceButton.dataset.resetPerformance));
    return;
  }
  const deleteButton = event.target.closest("[data-delete-shot]");
  if (deleteButton) {
    deleteShot(Number(deleteButton.dataset.deleteShot));
    return;
  }
  const regenerateWhiteButton = event.target.closest("[data-regenerate-white-shot]");
  if (regenerateWhiteButton) {
    regenerateWhiteModelShot(Number(regenerateWhiteButton.dataset.regenerateWhiteShot));
    return;
  }
  const regenerateButton = event.target.closest("[data-regenerate-shot]");
  if (regenerateButton) {
    regenerateShot(Number(regenerateButton.dataset.regenerateShot));
    return;
  }
  const approveCompositionButton = event.target.closest("[data-approve-composition-retry]");
  if (approveCompositionButton) {
    approveCompositionRetry(Number(approveCompositionButton.dataset.approveCompositionRetry));
    return;
  }
  const approveWhiteButton = event.target.closest("[data-approve-white-model]");
  if (approveWhiteButton) {
    approveWhiteModel(Number(approveWhiteButton.dataset.approveWhiteModel));
    return;
  }
  const skipShotButton = event.target.closest("[data-skip-shot]");
  if (skipShotButton) {
    skipShotAndContinue(Number(skipShotButton.dataset.skipShot));
    return;
  }
  const restoreSkippedButton = event.target.closest("[data-restore-skipped-shot]");
  if (restoreSkippedButton) {
    restoreSkippedShot(Number(restoreSkippedButton.dataset.restoreSkippedShot));
    return;
  }
  const addButton = event.target.closest("[data-add-person]");
  if (addButton) {
    const shotIndex = Number(addButton.dataset.addPerson); const current = [...castFor(shotIndex)];
    if (current.length >= Math.min(4, state.actorCount)) return toast("请先在上方角色库增加人物，再为本镜头增加原片人物槽位。", true);
    const unused = Array.from({ length: state.actorCount }, (_value, offset) => offset + 1).find((value) => !current.includes(value));
    if (unused) current.push(unused);
    state.shotCasts.set(shotIndex, current); state.manualCasts.add(shotIndex);
    refreshShotCast(shotIndex); invalidateCastConfirmation(); return;
  }
  const removeButton = event.target.closest("[data-remove-person]");
  if (removeButton) {
    const shotIndex = Number(removeButton.dataset.removePerson); const current = [...castFor(shotIndex)];
    current.pop(); state.shotCasts.set(shotIndex, current); state.manualCasts.add(shotIndex);
    refreshShotCast(shotIndex); invalidateCastConfirmation(); return;
  }
  const clearButton = event.target.closest("[data-clear-cast]");
  if (clearButton) {
    const shotIndex = Number(clearButton.dataset.clearCast); state.shotCasts.set(shotIndex, []); state.manualCasts.add(shotIndex);
    refreshShotCast(shotIndex); invalidateCastConfirmation(); return;
  }
  const link = event.target.closest(".shot-download");
  if (link) { event.preventDefault(); saveFile(link.dataset.url, link.dataset.name, link.dataset.type); }
});

$("#confirmCast").addEventListener("change", () => {
  if ($("#confirmCast").checked && [...state.shotCasts.values()].some((ids) => ids.some((id) => !id))) {
    $("#confirmCast").checked = false;
    state.castConfirmed = false;
    updateCostAndButton();
    return toast("仍有人物尚未选择，请先逐镜核对人物对应。", true);
  }
  if ($("#confirmCast").checked && generationStrategy() !== WHOLE_GENERATION_STRATEGY && !sceneMappingsComplete()) {
    $("#confirmCast").checked = false;
    state.castConfirmed = false;
    updateCostAndButton();
    return toast("仍有分镜没有匹配新场景，请先自动匹配或逐镜选择。", true);
  }
  state.castConfirmed = $("#confirmCast").checked;
  updateCostAndButton();
  if (state.castConfirmed) $("#generateBtn").scrollIntoView({ behavior: "smooth", block: "center" });
});

$("#finalStyleLibrary").addEventListener("click", (event) => {
  const button = event.target.closest("[data-final-style]");
  if (!button) return;
  renderFinalStyleLibrary(button.dataset.finalStyle);
  updateCostAndButton();
});

$("#finalDownloadLink").addEventListener("click", (event) => {
  event.preventDefault(); const link = $("#finalDownloadLink");
  if (link.dataset.downloadUrl) saveFile(link.dataset.downloadUrl, link.dataset.suggestedName, "video");
});

$("#downloadCenter").addEventListener("click", (event) => {
  const link = event.target.closest("[data-result-download-url]");
  if (!link) return;
  event.preventDefault();
  saveFile(link.dataset.resultDownloadUrl, link.dataset.resultDownloadName, "video");
});

function initializeWorkflowWorkspace() {
  const main = document.querySelector("main");
  const form = $("#longWorkflowForm");
  const taskPanel = document.querySelector(".task-panel");
  const archivePanel = $("#archives");
  if (!main || !form || !taskPanel || !archivePanel || $("#workflowSidebar")) return;
  taskPanel.id = "progress";
  const shell = document.createElement("div");
  shell.className = "real-workflow-shell";
  const aside = document.createElement("aside");
  aside.id = "workflowSidebar";
  aside.className = "workflow-sidebar";
  aside.innerHTML = `<div class="workflow-sidebar-title"><span>WORKFLOW</span><b>真实人物复刻流程</b><small>点击步骤进入对应操作页</small></div>
    <nav class="workflow-step-nav" aria-label="项目步骤">
      <button type="button" data-workflow-step="source"><i>01</i><span><b>上传原片</b><small>视频与切点</small></span></button>
      <button type="button" data-workflow-step="shots"><i>02</i><span><b>分析分镜</b><small>镜头与人数</small></span></button>
      <button type="button" data-workflow-step="preprocess"><i>03</i><span><b>台词与白模</b><small>打码、校订、母版</small></span></button>
      <button type="button" data-workflow-step="references"><i>04</i><span><b>人物与场景</b><small>角色库与素材</small></span></button>
      <button type="button" data-workflow-step="generate"><i>05</i><span><b>生成成片</b><small>逐镜 / 整段模式</small></span></button>
      <button type="button" data-workflow-step="progress"><i>06</i><span><b>进度与结果</b><small>分镜预览与下载</small></span></button>
      <button type="button" data-workflow-step="archives"><i>07</i><span><b>缓存与存档</b><small>保存和恢复</small></span></button>
    </nav>`;
  const stage = document.createElement("div");
  stage.className = "workflow-main-stage";
  main.insertBefore(shell, form);
  shell.append(aside, stage);
  stage.append(form, taskPanel, archivePanel);
  $("#source")?.classList.add("workflow-step-panel");
  $("#shots")?.classList.add("workflow-step-panel");
  $("#preprocess")?.classList.add("workflow-step-panel");
  $("#references")?.classList.add("workflow-step-panel");
  $("#generate")?.classList.add("workflow-step-panel");
  taskPanel.classList.add("workflow-step-panel");
  archivePanel.classList.add("workflow-step-panel");
  aside.addEventListener("click", (event) => {
    const button = event.target.closest("[data-workflow-step]");
    if (button) showWorkflowStep(button.dataset.workflowStep);
  });
  const requested = location.hash.replace(/^#/, "");
  showWorkflowStep(["source", "shots", "preprocess", "references", "generate", "progress", "archives"].includes(requested) ? requested : "source", false);
  document.body.classList.add("workflow-nav-ready");
}

function showWorkflowStep(step, updateHash = true) {
  const target = document.getElementById(step);
  if (!target?.classList.contains("workflow-step-panel")) return;
  state.activeWorkflowStep = step;
  document.querySelectorAll(".workflow-step-panel").forEach((panel) => panel.classList.toggle("active", panel.id === step));
  document.querySelectorAll("[data-workflow-step]").forEach((button) => button.classList.toggle("active", button.dataset.workflowStep === step));
  if (updateHash) history.replaceState(null, "", `#${step}`);
  window.scrollTo({ top: document.querySelector(".real-workflow-shell")?.offsetTop - 18 || 0, behavior: "smooth" });
  scheduleDraftCache();
}

function draftSnapshot() {
  const fields = [
    "sensitivity", "manualCuts", "blockSize", "faceScoreThreshold", "whiteModelRatio",
    "imageModel", "blurRange", "scenePrompt", "prompt", "finalStyle", "model", "resolution",
    "ratio", "wholeSceneReference",
  ];
  const values = {};
  fields.forEach((id) => {
    const node = document.getElementById(id);
    if (node) values[id] = node.value;
  });
  return {
    version: 2,
    savedAt: new Date().toISOString(),
    jobId: state.jobId || "",
    activeStep: state.activeWorkflowStep,
    generationStrategy: generationStrategy(),
    values,
    checks: {
      deleteTos: $("#deleteTos")?.checked === true,
      requireWhiteModel: $("#requireWhiteModel")?.checked === true,
    },
  };
}

function realDraftCacheKey() {
  const workspaceId = window.DepthFlowWorkspaces?.active()?.id || "default";
  return `${REAL_DRAFT_CACHE_KEY}.${workspaceId}`;
}

function saveDraftCache() {
  try {
    const draft = draftSnapshot();
    localStorage.setItem(realDraftCacheKey(), JSON.stringify(draft));
    if ($("#draftCacheState")) {
      const modeLabel = draft.generationStrategy === WHOLE_GENERATION_STRATEGY ? "整段白模模式" : "逐分镜模式";
      $("#draftCacheState").textContent = `已自动保存 · ${modeLabel} · ${new Date().toLocaleTimeString("zh-CN", { hour: "2-digit", minute: "2-digit" })}`;
    }
  } catch (_error) {
    if ($("#draftCacheState")) $("#draftCacheState").textContent = "浏览器缓存不可用；项目文件仍保存在本地任务目录。";
  }
}

function scheduleDraftCache() {
  if (!state.draftReady) return;
  clearTimeout(state.draftTimer);
  state.draftTimer = setTimeout(saveDraftCache, 350);
}

function restoreDraftCache() {
  try {
    const raw = localStorage.getItem(realDraftCacheKey());
    if (!raw) {
      $("#draftCacheState").textContent = "尚无页面参数缓存；修改任一设置后会自动保存。";
      return false;
    }
    const draft = JSON.parse(raw);
    if (!draft || draft.version !== 2) return false;
    const strategyInput = document.querySelector(`input[name="generationStrategy"][value="${draft.generationStrategy}"]`);
    if (strategyInput) strategyInput.checked = true;
    const modelValue = draft.values?.model;
    if (modelValue && [...$("#model").options].some((option) => option.value === modelValue)) $("#model").value = modelValue;
    syncFinalModelOptions(draft.values?.resolution || "720p");
    Object.entries(draft.values || {}).forEach(([id, value]) => {
      try {
        const node = document.getElementById(id);
        if (!node || id === "model") return;
        if (node.tagName === "SELECT") {
          if ([...node.options].some((option) => option.value === value)) node.value = value;
        } else {
          node.value = value;
        }
      } catch (_fieldError) { /* Ignore one stale field without discarding the rest of the draft. */ }
    });
    if ($("#deleteTos")) $("#deleteTos").checked = draft.checks?.deleteTos !== false;
    if ($("#requireWhiteModel")) $("#requireWhiteModel").checked = draft.checks?.requireWhiteModel !== false;
    syncGenerationStrategy();
    if (draft.activeStep) showWorkflowStep(draft.activeStep, false);
    $("#draftCacheState").textContent = `已恢复页面参数 · ${new Date(draft.savedAt).toLocaleString("zh-CN")}`;
    return true;
  } catch (_error) {
    $("#draftCacheState").textContent = "旧缓存无法读取，已忽略；不会影响任务文件。";
    return false;
  }
}

function clearDraftCache() {
  try { localStorage.removeItem(realDraftCacheKey()); } catch (_error) { /* ignore */ }
  $("#draftCacheState").textContent = "页面参数缓存已清除；任务文件和命名存档未删除。";
  toast("已清除页面参数缓存，不会删除项目任务或成片。", false);
}

function renderArchives() {
  const select = $("#archiveList");
  const current = select.value;
  select.innerHTML = state.archives.length
    ? `<option value="">选择一个存档</option>${state.archives.map((item) => `<option value="${escapeHtml(item.id)}">${escapeHtml(item.name)} · ${item.shot_count}镜 · ${escapeHtml((item.saved_at || "").replace("T", " "))}</option>`).join("")}`
    : '<option value="">暂无命名存档</option>';
  if (state.archives.some((item) => item.id === current)) select.value = current;
  const selected = Boolean(select.value);
  $("#restoreArchiveBtn").disabled = !selected;
  $("#deleteArchiveBtn").disabled = !selected;
}

async function loadArchives() {
  try {
    const workspaceId = window.DepthFlowWorkspaces?.active()?.id || "";
    const result = await api(`${LONG_API}/archives${workspaceId ? `?workspace_id=${encodeURIComponent(workspaceId)}` : ""}`);
    state.archives = result.archives || [];
    renderArchives();
  } catch (error) {
    toast(`存档列表读取失败：${error.message}`, true);
  }
}

async function saveArchive() {
  if (!state.jobId) return toast("请先完成或打开一个真实人物复刻任务。", true);
  try {
    const result = await api(`${LONG_API}/archives`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ source_job_id: state.jobId, name: $("#archiveName").value.trim() }),
    });
    $("#archiveName").value = "";
    await loadArchives();
    $("#archiveList").value = result.archive.id;
    renderArchives();
    toast("当前任务已保存为命名存档。", false);
  } catch (error) { toast(error.message, true); }
}

async function restoreArchive() {
  const archiveId = $("#archiveList").value;
  const archive = state.archives.find((item) => item.id === archiveId);
  if (!archive) return;
  if (!(await confirmAction(`恢复存档“${archive.name}”吗？\n\n页面将切回该存档当时的人物、场景、白模、分镜状态和已有结果。当前任务文件不会被删除。`, { title: "恢复命名存档", confirmLabel: "确认恢复" }))) return;
  try {
    setBusy(true);
    const job = await api(`${LONG_API}/archives/${archiveId}/restore`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ workspace_id: window.DepthFlowWorkspaces?.active()?.id || "" }),
    });
    render(job, true);
    showWorkflowStep("progress");
    toast(`已恢复存档：${archive.name}`, false);
  } catch (error) { toast(error.message, true); }
  finally { setBusy(false); }
}

async function deleteArchive() {
  const archiveId = $("#archiveList").value;
  const archive = state.archives.find((item) => item.id === archiveId);
  if (!archive || !(await confirmAction(`永久删除存档“${archive.name}”吗？\n不会删除原任务目录或已生成视频。`, { title: "删除命名存档", confirmLabel: "永久删除", danger: true }))) return;
  try {
    const workspaceId = window.DepthFlowWorkspaces?.active()?.id || "";
    const result = await api(`${LONG_API}/archives/${archiveId}${workspaceId ? `?workspace_id=${encodeURIComponent(workspaceId)}` : ""}`, { method: "DELETE" });
    state.archives = result.archives || [];
    renderArchives();
    toast("命名存档已删除；原任务文件仍保留。", false);
  } catch (error) { toast(error.message, true); }
}

async function loadConfig() {
  try {
    if (window.DepthFlowWorkspaces) await window.DepthFlowWorkspaces.ready;
    const config = await api("/api/config"); state.config = config;
    $("#scenePrompt").value = config.long_scene_prompt; $("#prompt").value = config.long_video_prompt;
    const modelOptions = config.real_final_video_models || [];
    if (modelOptions.length) {
      $("#model").innerHTML = modelOptions.map((item) => `<option value="${escapeHtml(item.model)}">${escapeHtml(item.label)}</option>`).join("");
    }
    $("#model").value = modelOptions.some((item) => item.model === config.model) ? config.model : (modelOptions[0]?.model || config.model);
    syncFinalModelOptions("720p");
    $("#imageModel").value = config.image_model;
    const platformMode = config.plugin_service_mode === "platform";
    badge("#arkBadge", config.ark_ready, platformMode ? "平台服务已就绪" : "API 已配置", platformMode ? "平台服务待配置" : "API 未配置");
    badge("#channelBadge", config.temporary_upload_ready, platformMode ? "平台素材服务可用" : "免费临时通道可用", platformMode ? "平台素材服务待配置" : "临时通道不可用");
    const activeWorkspace = window.DepthFlowWorkspaces?.active();
    const latest = activeWorkspace?.job_id
      ? await api(`/api/jobs/${activeWorkspace.job_id}`)
      : (window.DepthFlowWorkspaces ? null : config.latest_real_long_video);
    const previousStyle = latest?.shots?.find((shot) => shot.final_style_id)?.final_style_id;
    const savedStyle = previousStyle && previousStyle !== "match_character" ? previousStyle : REAL_DEFAULT_FINAL_STYLE;
    renderFinalStyleLibrary(savedStyle);
    state.lastJob = null;
    await loadCharacterLibrary();
    if (latest) {
      render(latest, latest.kind !== "long_generate");
      if (latest.generation_strategy === WHOLE_GENERATION_STRATEGY) $("#generationStrategyWhole").checked = true;
      if (["queued", "running"].includes(latest.status)) {
        setBusy(true);
        monitor(latest.id, monitorModeForJob(latest), { scroll: false });
      }
    }
    else {
      state.actorCount = 1;
      renderActorList([]);
      initializeSceneLibrary(null);
    }
    const draftRestored = restoreDraftCache();
    if (!draftRestored) syncGenerationStrategy({ keepResolution: true });
    state.draftReady = true;
    if (!draftRestored) scheduleDraftCache();
    await loadArchives();
  } catch (error) { toast(error.message, true); renderActorList([]); }
}

bind("#referenceVideo", "#videoFileName");
bind("#arkCharacterFile", "#arkCharacterFileName");
$("#model").addEventListener("change", () => { syncFinalModelOptions(); updateCostAndButton(); scheduleDraftCache(); });
document.querySelectorAll('input[name="generationStrategy"]').forEach((input) => input.addEventListener("change", () => syncGenerationStrategy({ keepResolution: true })));
$("#wholeSceneReference").addEventListener("change", () => { updateCostAndButton(); scheduleDraftCache(); });
$("#refreshCharacterLibraryBtn").addEventListener("click", () => loadCharacterLibrary({ rerenderActors: true }));
$("#createRealCharacterGroupBtn").addEventListener("click", createRealCharacterGroup);
$("#quickCreateCharacterGroupBtn").addEventListener("click", () => openQuickCharacterGroupDialog("#arkCharacterGroup"));
$("#confirmCreateCharacterGroupBtn").addEventListener("click", confirmQuickCharacterGroup);
$("#cancelCharacterGroupDialogBtn").addEventListener("click", closeQuickCharacterGroupDialog);
$("#closeCharacterGroupDialogBtn").addEventListener("click", closeQuickCharacterGroupDialog);
$("#quickCharacterGroupForm").addEventListener("submit", (event) => { event.preventDefault(); confirmQuickCharacterGroup(); });
$("#renameCharacterGroupBtn").addEventListener("click", renameSelectedCharacterGroup);
$("#deleteCharacterGroupBtn").addEventListener("click", deleteSelectedCharacterGroup);
$("#arkCharacterGroup").addEventListener("change", syncCharacterGroupActions);
$("#uploadCharacterAssetBtn").addEventListener("click", uploadCharacterAsset);
$("#arkCharacterAssetList").addEventListener("click", (event) => {
  const button = event.target.closest("[data-delete-character-asset]");
  if (button) deleteCharacterAsset(button.dataset.deleteCharacterAsset);
});
$("#prepareRealActorsBtn").addEventListener("click", prepareRealActors);
$("#realPersonAuthorized").addEventListener("change", updatePipelineButtons);
$("#sceneGroupList").addEventListener("change", (event) => {
  const fileInput = event.target.closest("[data-scene-files]");
  if (fileInput) {
    const group = sceneGroupById(fileInput.dataset.sceneFiles);
    if (!group) return;
    const files = Array.from(fileInput.files || []).slice(0, 6);
    if (!files.length) return;
    group.images = files.map((file, offset) => ({ index: offset + 1, name: file.name, file, url: "" }));
    for (const [shotIndex, assignment] of state.shotScenes) {
      if (assignment.groupId === group.id && assignment.imageIndex > group.images.length) state.shotScenes.delete(shotIndex);
    }
    renderSceneLibrary();
    if (state.lastJob?.shots?.length) autoMatchScenes(false);
    else invalidateCastConfirmation();
    return;
  }
  const nameInput = event.target.closest("[data-scene-name]");
  if (nameInput) {
    const group = sceneGroupById(nameInput.dataset.sceneName);
    if (group) group.name = nameInput.value.trim() || "未命名场景";
    renderShotGrid(); invalidateCastConfirmation(); return;
  }
  const descriptionInput = event.target.closest("[data-scene-description]");
  if (descriptionInput) {
    const group = sceneGroupById(descriptionInput.dataset.sceneDescription);
    if (group) group.description = descriptionInput.value.trim();
    invalidateCastConfirmation();
  }
});
$("#sceneGroupList").addEventListener("click", (event) => {
  const button = event.target.closest("[data-remove-scene-group]");
  if (!button || state.sceneGroups.length <= 1) return;
  const groupId = button.dataset.removeSceneGroup;
  state.sceneGroups = state.sceneGroups.filter((group) => group.id !== groupId);
  for (const [shotIndex, assignment] of state.shotScenes) {
    if (assignment.groupId === groupId) state.shotScenes.delete(shotIndex);
  }
  renderSceneLibrary(); renderShotGrid(); invalidateCastConfirmation();
});
$("#addSceneGroupBtn").addEventListener("click", () => {
  if (state.sceneGroups.length >= 20) return toast("场景库最多支持20个场景组。", true);
  state.sceneGroups.push(normalizeSceneGroup(null, state.sceneGroups.length + 1));
  renderSceneLibrary(); invalidateCastConfirmation();
});
$("#autoMatchScenesBtn").addEventListener("click", () => autoMatchScenes(true));
$("#sensitivity").addEventListener("input", () => { $("#sensitivityValue").textContent = $("#sensitivity").value; });
$("#analyzeBtn").addEventListener("click", analyze); $("#generateBtn").addEventListener("click", generate);
$("#pauseGenerateBtn").addEventListener("click", pauseGeneration);
$("#regenerateAllBtn").addEventListener("click", regenerateAll);
$("#mosaicBtn").addEventListener("click", prepareMosaic);
$("#performanceBtn").addEventListener("click", analyzePerformance);
$("#continuityReviewLink").addEventListener("click", (event) => { event.preventDefault(); openCastReview(); });
$("#whiteModelBtn").addEventListener("click", generateWhiteModel);
$("#performanceConsent").addEventListener("change", updatePipelineButtons);
$("#privacyReviewConfirmed").addEventListener("change", updatePipelineButtons);
$("#requireWhiteModel").addEventListener("change", updateCostAndButton);
$("#addActorBtn").addEventListener("click", addActor); $("#removeActorBtn").addEventListener("click", removeActor);
$("#openFolderBtn").addEventListener("click", openFolder);
$("#saveArchiveBtn").addEventListener("click", saveArchive);
$("#archiveList").addEventListener("change", renderArchives);
$("#restoreArchiveBtn").addEventListener("click", restoreArchive);
$("#deleteArchiveBtn").addEventListener("click", deleteArchive);
$("#clearDraftCacheBtn").addEventListener("click", clearDraftCache);
$("#resetScenePromptBtn").addEventListener("click", () => { $("#scenePrompt").value = state.config.long_scene_prompt; });
$("#resetPromptBtn").addEventListener("click", () => { $("#prompt").value = state.config.long_video_prompt; });
$("#mosaicDownloadLink").addEventListener("click", (event) => {
  event.preventDefault(); const link = $("#mosaicDownloadLink");
  if (link.dataset.downloadUrl) saveFile(link.dataset.downloadUrl, link.dataset.suggestedName, "video");
});
$("#whiteModelDownloadLink").addEventListener("click", (event) => {
  event.preventDefault(); const link = $("#whiteModelDownloadLink");
  if (link.dataset.downloadUrl) saveFile(link.dataset.downloadUrl, link.dataset.suggestedName, "video");
});
$("#longWorkflowForm").addEventListener("input", scheduleDraftCache);
$("#longWorkflowForm").addEventListener("change", scheduleDraftCache);
initializeWorkflowWorkspace();
loadConfig();
