...
WHITE_MODEL_FINAL_MASTER_CONSTRAINT = (
    ...
    "白模的白色工作服、手套与平底鞋仅为动作占位，最终服饰按对应服装参考完整替换。"
)
...
# All three projects use a fully dressed animation proxy. This specifies the
# intended output; it does not diagnose or override a provider moderation result.
WHITE_MODEL_VISUAL_CONSTRAINT = (
    "逐帧保留所有人物的数量、动作时序、身体姿态、手势、头部朝向、视线、嘴部开合、表情节奏、站位、尺度、"
    "遮挡、互动和移动路径；保留每次切镜、镜头机位、焦段、景别、裁切、透视、构图、对焦与运镜轨迹。"
    "所有人物统一替换为非写实的纯白三维动画人偶，使用无身份特征的光滑椭圆头部和简化几何面部。"
    "每个人偶始终穿着宽松、不透明的纯白长袖长裤连体工作服，具有清楚的衣领、袖口、裤脚和少量自然衣褶，"
    "搭配纯白手套与纯白平底鞋；全程保持完整穿着，服装轮廓不紧贴身体，不还原原片服饰的款式、颜色或标志。"
    "头部保持光滑无毛发，眼部、鼻部、嘴部和下颌用简洁几何结构表达视线、口型与表情，不复制原人物五官。"
    "人偶和工作服统一为纯白哑光材质，以柔和明暗和接触阴影表达体积，手部、关节和人物边界清楚稳定。"
    "仅保留动作中正在使用的必要道具，并简化为纯白几何模型；其他环境替换为均匀、干净、无渐变的标准绿幕 #00B140，"
    "边缘清晰且没有绿色反光。保持人物数量和肢体结构稳定，避免重影、闪烁、粘连、动作漂移或新增动作。"
    "画面中严禁出现或保留任何字幕、台词文字、标题、时间码、数字、招牌文字、屏幕界面文字、Logo、品牌标识、"
    "平台角标或水印；台词只表现为嘴部动作，不显示文字。输出静音、清晰稳定的白色动画人偶绿幕动作参考视频。"
)
DEFAULT_WHITE_MODEL_PROMPT = (
    "参考@视频1，将当前分镜制作成穿着完整的白色动画人偶动作母版。" + WHITE_MODEL_VISUAL_CONSTRAINT
)
WARDROBE_SAFE_WHITE_MODEL_PROMPT = (
    "参考@视频1，将整段视频制作成穿着完整的白色动画人偶动作母版。" + WHITE_MODEL_VISUAL_CONSTRAINT
)
...
def build_wardrobe_swap_prompt(mode: str) -> str:
    ...
    shared = (
        ...
        "不得把白模的绿幕、白色材质、工作服、手套、平底鞋或无身份五官带入成片。最终输出自然、清晰、稳定的正常彩色视频。"
...
def build_white_model_correction_prompt(qa: dict[str, Any]) -> str:
    ...
    if "彩色" in reasons or "服装" in reasons or "头发" in reasons:
        directives.append("将残留的原片外观替换为无身份特征的白色动画人偶与纯白工作服，保留完整穿着")
    if "文字" in reasons or "字幕" in reasons or "Logo" in reasons:
        directives.append("删除全部字幕、数字、时间码、Logo和水印，任何帧都不得出现字符")
    if not directives:
        directives.append("严格恢复@视频1人物占位与动作，并输出无文字的白色动画人偶绿幕视频")
    directives.append("每个人偶始终穿着宽松不透明的纯白长袖长裤连体工作服、纯白手套与平底鞋，保留衣领、袖口与裤脚")
