...
(() => {
  ...
  if (!parent || !/^[a-f0-9]{64}$/.test(state || '')) {
    status.textContent = '请回 CZMIYOU 统一登录后的「我的应用」，点击衣装智换。';
    return;
  }
  const timer = setTimeout(() => {accepted = true; status.textContent = '连接已过期，请回统一登录页面重新点击衣装智换。';}, 55000);
  window.addEventListener('message', async event => {
    ...
    accepted = true;
    clearTimeout(timer);
    status.textContent = '正在验证统一登录及衣装智换时间卡…';
    try {
      ...
      window.opener = null;
      location.replace('/projects/wardrobe');
    } catch (_) {
      parent.postMessage({type:'YZZH_LOGIN_FAILED',state}, loginOrigin);
      status.textContent = '未能完成账号验证，已有任务未删除。请返回统一登录页面重新登录，再点击衣装智换。';
    }
  });
