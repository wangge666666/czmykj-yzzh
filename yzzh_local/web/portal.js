...
(() => {
  ...
  function show(message, view = 'settings') {
    ...
    shadow.querySelector('.panel-title').textContent = view === 'approval' ? '确认本次操作' : '账号与创作设置';
  ...
  async function refresh() {
    try {
      ...
      shadow.querySelector('.login-destination').textContent = `登录信息只发往：${state.health.login_url}。不要把密码或 Key 发到聊天中。`;
      button.textContent = current ? '账号与创作设置' : '登录并开始创作';
      if (current && (String(current.user_id) !== owner || state.settings.session_revision !== context)) {
        hasAttention = false; attentionSignature = ''; pending.replaceChildren();
        clearReadiness('账号已切换，请刷新后重新检查。');
      ...
      settingsForm.hidden = !current;
      if (!current) {hasAttention = false; attentionSignature = ''; pending.replaceChildren(); clearReadiness('登录后自动检查本地基础功能。'); return;}
      if (!readinessKey) {
        readinessKey = `${owner}:${context}`;
        checkReadiness();
      }
      syncApiBadge(state.settings);
      for (const [name,,type] of fields) {
        if (type === 'password') settingsForm.elements[name].placeholder = state.settings.fields[name] === true
      ...
      const signature = JSON.stringify([approvals.pending.map(item => [item.id, item.state]), approvals.unresolved.map(item => [item.id, item.state, item.diagnosis])]);
      ...
      for (const item of approvals.pending) {
        ...
        const upload = item.summary.destination === 'Litterbox 临时素材托管';
        const title = document.createElement('h3'); title.textContent = upload ? '确认上传本次素材' : '确认当前云端操作'; section.append(title);
        const description = document.createElement('p');
        description.textContent = upload ? `${(item.summary.bytes / 1048576).toFixed(1)} MB 素材将临时上传至 Litterbox，供人物审核或视频生成读取。持链接可访问，服务声明约 3 天过期，插件不能提前删除。请确认你有权上传，且不包含保密素材。` : `${item.summary.destination}：${item.summary.cost}`;
        ...
        for (const approved of [true,false]) {
          ...
          action.onclick = async () => {action.disabled = true; try {await api('/_plugin/decision', {owner:Number(owner), session:context, id:item.id, approved}); await refresh();} catch(e) {show(e.message, 'approval');} finally {action.disabled = false;}};
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    <div class="overlay" hidden><div class="panel" role="dialog" aria-modal="true" aria-label="插件账号与设置">
      ...
      <p>沿用原来的创作流程。时间卡控制插件使用权，生成费用由自己的模型账号承担，不扣 CZMIYOU 余额。</p>
      ...
      <div class="approval-view" hidden><p>本次操作将沿用已保存在本机的配置，无需重新填写 API Key。请核对下面的素材去向或生成费用后决定是否继续。</p><div class="pending"></div></div>
    </div></div>`;
    ...
    shadow.querySelector('.logout').onclick = async () => {try {await api('/api/logout',{}); location.reload();} catch(e) {show(e.message);}};
